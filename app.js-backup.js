...
// const LEAVE_BUFFER = 50; // extra margin backward - not used after 50m rule
...
class VesselStateManager extends EventEmitter {
  ...
  _performCleanup(mmsi) {
    ...
    if (age > timeout) {
      ...
      } else if (Date.now() - vessel.graceStartTime > 30000) {
  ...
  _updateSpeedHistory(history = [], currentSpeed) {
    ...
    const filtered = history
      .filter((entry) => now - entry.timestamp < MAX_AGE)
      .slice(-MAX_HISTORY + 1);
...
class BridgeMonitor extends EventEmitter {
  constructor(bridges, vesselManager, logger, boundingBox = null) {
    super();
    this.bridges = bridges;
    this.vesselManager = vesselManager;
    this.logger = logger;
    this.boundingBox = boundingBox; // nytt fält för bounding box
  ...
  /**
   * Public setter som kan kallas från appen när BB ändras
   */
  setBoundingBox(box) {
  ...
  _handleVesselUpdate(vessel, oldData) {
    ...
    if (nearestBridge) {
      ...
      // Protection against bounce in Järnvägsbron/Stridsbergsbron overlap
      const last = vessel.nearBridge;
      if (last && last !== bridgeId) {
        ...
        // Byt bara om ny bro är ≥20% närmare
        if (distance > lastDist * 0.8) {
          bridgeId = last;
          distance = lastDist;
        }
      }
      ...
      if (distance <= APPROACH_RADIUS) {
        ...
        // Waiting detection logic
        const WAIT_SPEED_KN = 0.2;
        const WAIT_TIME_SEC = 180; // 3 minuter

        if (vessel.sog < WAIT_SPEED_KN && distance <= APPROACH_RADIUS) {
          if (!vessel.waitSince) vessel.waitSince = Date.now();
          if (Date.now() - vessel.waitSince > WAIT_TIME_SEC * 1000) {
        ...
        if (vessel.targetBridge) {
          ...
          if (targetId && vessel.nearBridge === targetId && distance < 50) {
      ...
      if (
        ...
        if (targetBridgeId) {
          ...
          // Secure bridge passage detector - track when vessel enters target radius
          if (targetDistance <= APPROACH_RADIUS) {
            vessel._wasInsideTarget = true;
          }

          // Mark as passed only if vessel was previously inside AND now has left the buffer
          if (
            vessel._wasInsideTarget
            && targetDistance > 50 // lämnat bron med minst 50 m
          ) {
            this.logger.debug(
              `🌉 [BRIDGE_PASSAGE] Fartyg ${vessel.mmsi} har passerat ${
                vessel.targetBridge
              } (avstånd: ${targetDistance.toFixed(0)}m > 50m)`,
            );
    ...
    const outsideBridgeZone = this._findNearestBridge(vessel).distance > APPROACH_RADIUS;
  ...
  checkVesselPosition(vessel) {
    ...
    // Check for bridge passages
    this._checkBridgePassages(vessel);
  }
  ...
  _analyzeApproach(vessel, bridge, distance) {
    ...
    const headingDiff = Math.abs(((vessel.cog - bearing + 180) % 360) - 180);
    ...
    const isWaiting = distance < 100 && vessel.sog < 0.5 && vessel.maxRecentSpeed > 2.0;
  ...
  _findTargetBridge(vessel, currentBridgeId) {
    ...
    if (this.userBridges.includes(currentBridgeId)) {
  ...
  _checkBridgePassages(vessel) {
    if (!vessel.passedBridges) vessel.passedBridges = [];

    for (const [bridgeId, bridge] of Object.entries(this.bridges)) {
      if (vessel.passedBridges.includes(bridgeId)) continue;
      ...
      const wasNear = this.vesselManager.bridgeVessels
        .get(bridgeId)
        ?.has(vessel.mmsi);

      if (wasNear && distance > bridge.radius + 50) {
        vessel.passedBridges.push(bridgeId);
        vessel.status = 'passed';
        this.emit('bridge:passed', { vessel, bridgeId, bridge });

        // Check if this is a user bridge and trigger UI refresh if so
        if (this.userBridges.includes(bridgeId)) {
          this.emit('vessel:user-bridge-passed', { vessel });
        }
  ...
  _isOnIncomingSide(vessel, bridge) {
    ...
    const diff = Math.abs(((bearing - vessel.cog + 180) % 360) - 180);
    return diff <= 90;
  }
  ...
  _isInsideBoundingBox(lat, lon) {
    if (!this.boundingBox) return true; // ingen ruta = acceptera allt
...
class AISConnectionManager extends EventEmitter {
  ...
  _scheduleReconnect() {
    ...
    const delay = Math.min(10000 * 1.5 ** this.reconnectAttempts, maxDelay);
...
class MessageGenerator {
  ...
  generateBridgeText(relevantBoats) {
    ...
    relevantBoats.forEach((boat, index) => {
      this.logger.debug(
        `🚢 [BRIDGE_TEXT] Båt ${index + 1}/${relevantBoats.length}:`,
        {
          mmsi: boat.mmsi,
          currentBridge: boat.currentBridge,
          targetBridge: boat.targetBridge,
          etaMinutes: boat.etaMinutes?.toFixed(1),
          isWaiting: boat.isWaiting,
          confidence: boat.confidence,
          distance: `${boat.distance?.toFixed(0)}m`,
        },
      );
    });

    const groups = this._groupByTargetBridge(relevantBoats);
    const phrases = [];
  ...
  _generatePhraseForBridge(bridgeName, boats) {
    ...
    this.logger.debug(`📈 [BRIDGE_TEXT] Fras-stats för ${bridgeName}:`, {
      ...
      closestBoat: {
        mmsi: closest.mmsi,
        etaMinutes: closest.etaMinutes?.toFixed(1),
    ...
    if (
      ...
      const suffix = eta ? `, beräknad öppning ${eta}` : '';
    ...
    if (count === 1) {
      ...
      if (additionalCount === 0) {
        ...
        if (waiting === 1) {
          phrase = `En båt väntar vid ${bridgeName}`;
        } else {
          phrase = `${waiting} båtar väntar vid ${bridgeName}`;
        }
      } else {
        ...
        phrase = `${waiting} båtar väntar vid ${bridgeName}, ${additionalText} på väg, beräknad broöppning ${eta}`;
      }
      this.logger.debug(
...
class ETACalculator {
  ...
  calculateETA(vessel, targetDistance, bridgeId, targetBridgeId) {
    ...
    const actualDistance = this.bridgeGaps[gapKey] || targetDistance;
    ...
    if (speedMs < 0.05) {
      ...
      return { minutes: Infinity, isWaiting: false };
    }

    let eta = actualDistance / speedMs / 60;
    this.logger.debug(
    ...
    // Apply caps based on distance
    const originalEta = eta;
    ...
    if (eta !== originalEta) {
      this.logger.debug(
        `📊 [ETA_CALC] ETA begränsad från ${originalEta.toFixed(
          1,
        )}min till ${eta.toFixed(1)}min baserat på avstånd`,
      );
    }
...
class AISBridgeApp extends Homey.App {
  ...
  _connectModuleEvents() {
    ...
    // Bridge events
    this.bridgeMonitor.on('bridge:approaching', (event) => {
      this._handleBridgeApproaching(event);
    });

    this.bridgeMonitor.on('bridge:passed', (event) => {
      this._handleBridgePassed(event);
    });

    // ETA change events for continuous UI updates
    this.bridgeMonitor.on('vessel:eta-changed', () => {
      this.debug('[UI] ETA changed - uppdaterar UI');
      this._updateUI();
    });

    // User bridge passed events for immediate UI refresh
    this.bridgeMonitor.on('vessel:user-bridge-passed', () => {
  ...
  async _startConnection() {
    ...
    // Skicka bounding-boxen till BridgeMonitor
    this.bridgeMonitor.setBoundingBox(this.aisConnection.boundingBox);
  }
  ...
  _findRelevantBoats() {
    ...
    for (const vessel of this.vesselManager.vessels.values()) {
      // Filter out passed vessels only when targetBridge === null
      if (vessel.status === 'passed' && vessel.targetBridge === null) continue;
      ...
      const targetBridge = this.bridges[targetBridgeId];
      const distanceToTarget = this.bridgeMonitor._haversine(
        vessel.lat,
        vessel.lon,
        targetBridge.lat,
        targetBridge.lon,
      ...
      let currentBridgeName = 'närområdet'; // better fallback than targetBridge
  ...
  _triggerBoatNearFlow(bridgeId, bridgeName, vesselName, direction = null) {
    ...
    this._boatNearTrigger
      ...
      .catch(this.error);
    this._boatNearTrigger.trigger(tokens, { bridge: 'any' }).catch(this.error);
  }
