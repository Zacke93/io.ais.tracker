...
class StatusService extends EventEmitter {
  constructor(bridgeRegistry, logger, systemCoordinator) {
    super();
  ...
  calculateETA(vessel, proximityData) {
    ...
    // CRITICAL FIX: Validate vessel has valid position data using helper function
    ...
    // Extra safety check using helper function
    ...
    try {
      ...
      // Validate distance calculation using helper function
    ...
    // Extra validation: Ensure speedMps is valid before division
    if (!Number.isFinite(speedMps) || speedMps <= 0) {
      this.logger.log(`⏰ [ETA_CALC] ${vessel.mmsi}: Invalid speed conversion (${speedMps}m/s) - returning null`);
      return null;
    }
    ...
    if (timeSeconds === null) {
      this.logger.log(`⏰ [ETA_CALC] ${vessel.mmsi}: Safe division failed (distance=${distance}m, speedMps=${speedMps}m/s) - returning null`);
      return null;
    }

    const timeMinutes = safeDivision(timeSeconds, 60);
    if (timeMinutes === null) {
      this.logger.log(`⏰ [ETA_CALC] ${vessel.mmsi}: Time conversion failed (timeSeconds=${timeSeconds}s) - returning null`);
      return null;
    }
  ...
  _isApproaching(vessel, proximityData) {
    ...
    if (stallbackabron) {
      ...
      if (distanceToStallbacka !== null && Number.isFinite(distanceToStallbacka)
          && distanceToStallbacka <= APPROACHING_RADIUS && distanceToStallbacka > APPROACH_RADIUS && vessel.sog > 0.5) {
