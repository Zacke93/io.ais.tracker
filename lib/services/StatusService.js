...
class StatusService extends EventEmitter {
  ...
  _isWaiting(vessel, proximityData) {
    ...
    const targetDistance = this._getDistanceToTargetBridge(vessel);
    const targetDistanceStr = targetDistance !== null ? `${targetDistance.toFixed(0)}` : 'undefined';
    this.logger.debug(`🔍 [WAITING_CHECK] ${vessel.mmsi}: Target distance = ${targetDistanceStr}m to ${vessel.targetBridge || 'null'}, threshold = ${APPROACH_RADIUS}m`);
