...
class StatusService extends EventEmitter {
  ...
  analyzeVesselStatus(vessel, proximityData, positionAnalysis = null) {
    ...
    const syntheticUnderHoldActive = Boolean(
      vessel._syntheticUnderBridgeUntil && vessel._syntheticUnderBridgeUntil > Date.now(),
    );
    ...
    } else if (hasRecentlyPassed && !syntheticUnderHoldActive) {
  ...
  _isUnderBridge(vessel, proximityData) {
    const now = Date.now();
    if (vessel._syntheticUnderBridgeUntil) {
      if (vessel._syntheticUnderBridgeUntil > now) {
        const syntheticBridge = vessel._syntheticUnderBridgeBridgeName || vessel.currentBridge || vessel.targetBridge;
        ...
        if (!vessel._syntheticUnderBridgeLogged || now - vessel._syntheticUnderBridgeLogged > 1000) {
          this.logger.debug(
            `🕒 [SYNTHETIC_UNDER_BRIDGE] ${vessel.mmsi}: Holding under-bridge state for ${syntheticBridge} `
            + `(${((vessel._syntheticUnderBridgeUntil - now) / 1000).toFixed(1)}s remaining)`,
          );
          vessel._syntheticUnderBridgeLogged = now;
        }
        return true;
      }
      vessel._syntheticUnderBridgeUntil = null;
      vessel._syntheticUnderBridgeBridgeName = null;
      ...
      if (nearestBridge && nearestBridge.name && nearestBridge.name !== vessel.targetBridge) {
        ...
        if (isIntermediateBridge) {
          ...
          if (this.passageLatchService
              && this.passageLatchService.shouldBlockStatus(vessel.mmsi.toString(), bridgeName, 'waiting')) {
  ...
  _isApproaching(vessel, proximityData) {
    ...
    if (targetDistance !== null) {
      ...
      if (this.passageLatchService
          && this.passageLatchService.shouldBlockStatus(vessel.mmsi.toString(), vessel.targetBridge, 'approaching')) {
    ...
    if (proximityData.nearestDistance <= approachThreshold && proximityData.nearestDistance > APPROACH_RADIUS) {
      ...
      if (nearestBridge && nearestBridge.name) {
        ...
        if (isIntermediateBridge) {
          ...
          if (this.passageLatchService
              && this.passageLatchService.shouldBlockStatus(vessel.mmsi.toString(), bridgeName, 'approaching')) {
