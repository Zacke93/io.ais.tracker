  ...
  _isUnderBridge(vessel, proximityData) {
    ...
    if (wasUnderBridge && !hysteresisWasReset) {
      if (!vessel._underBridgeSince) {
        ...
        // 17:03, gap 16:53→17:06 under Järnvägsbron). Frysning: skjut basen
        // framåt så varaktigheten står stilla på värdet vid gap-start;
        // när nästa position kommer återupptas ackumuleringen därifrån.
        // Äkta ankrat-under-bro (Bug #5-fallet) har färsk AIS och
        // force-clearas precis som förut.
        const UNDER_BRIDGE_FRESH_MS = 2 * 60 * 1000;
        const posAgeMs = Number.isFinite(vessel.lastPositionUpdate)
          ? now - vessel.lastPositionUpdate
          : Infinity;
        if (posAgeMs > UNDER_BRIDGE_FRESH_MS) {
          vessel._underBridgeSince = now - Math.max(
            0,
