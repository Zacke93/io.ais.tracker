...
class StatusService extends EventEmitter {
  ...
  _isUnderBridge(vessel, proximityData, positionAnalysis = null) {
    const now = Date.now();
    // SEGMENTBEVIS (V2, A/B-nattkörningen 2026-08-03): behåll FÖREGÅENDE
    // distinkta position så inträdessteget till under-bro-zonen kan prövas mot
    // brolinjen. Fångas här (före alla return-vägar) och bara när positionen
    // faktiskt ändrats — timer-passen (samma position, var 30:e sekund) får
    // annars segmentet att kollapsa till en punkt.
    ...
    if (Number.isFinite(vessel.lat) && Number.isFinite(vessel.lon)
        && (vessel.lat !== vessel._underBridgePrevLat || vessel.lon !== vessel._underBridgePrevLon)) {
   ...
   *  - andrakällan matar fartyget (utrullningsgrind, se constants.js —
   *    enkelkälleläget står facit-låst).
   *
   * @param {Object} vessel - fartygsobjektet (skrivs)
   * @param {string} bridgeName - bron zonbesöket gäller
   * @param {Object|null} prevFix - {lat, lon, sog} för föregående distinkta fix
  ...
  _noteUnderBridgeLineCross(vessel, bridgeName, prevFix, jumpTick = false) {
    ...
    // KÄLLGRIND: beviset kompenserar ett samplingsartefakt som andrakällan gör
    // 10× vanligare. I enkelkälleläget är beteendet facit-låst (fyra golden-
    // texter) och lämnas BIT-IDENTISKT tills omlåsning beslutats. Grinden
    // frågar om KÄLLNÄRVARO (senaste hub-fix inom TTL:n), inte om vilken källa
    // som råkade leverera just de två fixarna — se _isUnderBridge.
    if (UNDER_BRIDGE_CROSS_PROOF_REQUIRED_FEED
    ...
    if (!Number.isFinite(segM) || segM > UNDER_BRIDGE_CROSS_SEGMENT_MAX_M) return;
    ...
    const crossed = geometry.isDecisivelyOppositeBridgeSide(
    ...
    vessel._underBridgeEntryLat = prevFix.lat;
    vessel._underBridgeEntryLon = prevFix.lon;
    this.logger.debug(
      `🧵 [UNDER_BRIDGE_CROSS_PROOF] ${vessel.mmsi}: consecutive-fix segment (${Math.round(segM)}m) `
        ...
        if (isIntermediateBridge) {
          ...
          if (this._isDepartingPassedBridge(vessel, bridgeName, proximityData.nearestDistance)) {
            this.logger.debug(
              ...
              + `vessel is moving AWAY (${proximityData.nearestDistance.toFixed(0)}m) — blocking waiting`,
            );
            return false;
          }
