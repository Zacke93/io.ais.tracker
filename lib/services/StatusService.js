...
class StatusService extends EventEmitter {
  ...
  _isUnderBridge(vessel, proximityData, positionAnalysis = null) {
    ...
    if (vessel._underBridgeTimeoutBlockedBridge) {
      ...
      if (vessel.currentBridge === blockedName && Number.isFinite(vessel.distanceToCurrent)) {
        blockedDist = vessel.distanceToCurrent;
      } else if (vessel.targetBridge === blockedName) {
        const d = this._getDistanceToTargetBridge(vessel);
      ...
      if (passedAfterBlock
          || blockedDist === null
          || blockedDist > UNDER_BRIDGE_CLEAR_DISTANCE) {
