...
class StatusService extends EventEmitter {
  ...
  _isUnderBridge(vessel, proximityData) {
    ...
    if (stallbackabron) {
      ...
      // Apply hysteresis to Stallbackabron as well
      const stallbackaUnder = wasUnderBridge 
        ? distanceToStallbacka < UNDER_BRIDGE_CLEAR_DISTANCE  // Clear at 70m
        : distanceToStallbacka <= UNDER_BRIDGE_SET_DISTANCE;  // Set at 50m
        
      if (stallbackaUnder) {
        vessel._underBridgeLatched = true;
        this.logger.debug(`🌉 [STALLBACKA_UNDER] ${vessel.mmsi}: ${distanceToStallbacka.toFixed(0)}m from Stallbackabron -> under-bridge (hysteresis: ${wasUnderBridge ? 'HELD' : 'SET'})`);
        return true;
      }
    }

    // INTERMEDIATE BRIDGE CHECK: If vessel has currentBridge set and is very close to it
    if (vessel.currentBridge && vessel.distanceToCurrent !== null && vessel.distanceToCurrent !== undefined) {
      ...
      if (intermediateUnder) {
        vessel._underBridgeLatched = true;
        this.logger.debug(`🌉 [INTERMEDIATE_UNDER] ${vessel.mmsi}: ${vessel.distanceToCurrent.toFixed(0)}m from ${vessel.currentBridge} -> under-bridge (hysteresis: ${wasUnderBridge ? 'HELD' : 'SET'})`);
        return true;
      }
    }
    ...
    if (targetDistance !== null) {
      ...
      if (targetUnder) {
        vessel._underBridgeLatched = true;
        this.logger.debug(`🎯 [TARGET_UNDER] ${vessel.mmsi}: ${targetDistance.toFixed(0)}m from target -> under-bridge (hysteresis: ${wasUnderBridge ? 'HELD' : 'SET'})`);
        return true;
      }
    }
  ...
  _isWaiting(vessel, proximityData) {
    ...
    const targetDistance = this._getDistanceToTargetBridge(vessel);
    this.logger.debug(`🔍 [WAITING_CHECK] ${vessel.mmsi}: Target distance = ${targetDistance?.toFixed(0)}m to ${vessel.targetBridge}, threshold = ${APPROACH_RADIUS}m`);
  ...
  _isApproaching(vessel, proximityData) {
    ...
    if (proximityData.nearestDistance <= APPROACHING_RADIUS && proximityData.nearestDistance > APPROACH_RADIUS && vessel.sog > 0.5) {
      const { nearestBridge } = proximityData;
