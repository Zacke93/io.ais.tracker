...
class GPSJumpGateService {
  ...
  confirmStableCandidates(vesselId, vessel) {
    ...
    for (const candidate of candidates) {
      ...
      if (age >= this._confirmationPeriod) {
        // Etapp 0: äkta fixseparation (samma källa, båda snapshotter med
        // fixTs) skickas som egen parameter — age (mottagningsålder) styr
        // OFÖRÄNDRAT TTL:n ovan och bekräftelseperioden (domäninvarianten).
        const fixElapsedMs = (candidate.vesselState
          && candidate.vesselState.fixFeed
          && candidate.vesselState.fixFeed === vessel.fixFeed
          && Number.isFinite(candidate.vesselState.fixTs)
          && Number.isFinite(vessel.fixTs))
          ? vessel.fixTs - candidate.vesselState.fixTs
          : null;
  ...
  _isVesselStable(oldState, newState, elapsedMs = null, fixElapsedMs = null) {
    ...
    // Etapp 0: fysikfönstret mäter FIXSEPARATION när den finns (samma källa,
    // båda snapshotter bär fixTs) — en pollande källas batch kan bära ett
    // fix som är äldre än mottagningen, och mottagningsåldern ger då fel
    // fönster. age/elapsedMs styr oförändrat TTL + bekräftelseperiod.
