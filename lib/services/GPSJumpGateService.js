...
const { MOORING_DETECTION } = require('../constants');

class GPSJumpGateService {
  static BASE_GATE_TIMEOUT_MS = 30 * 1000;
  ...
  constructor(logger, systemCoordinator) {
    ...
    this._gateTimeout = GPSJumpGateService.BASE_GATE_TIMEOUT_MS; // 30 sekunder
    ...
    this._candidateTtl = GPSJumpAnalyzer.PASSAGE_RECOVERY_MAX_AGE_MS;
  ...
  shouldBlockPassageDetection(vesselId, vessel, bridgeName) {
    // Fart över stillaståendetröskeln gör inte en orimlig förflyttning
    // tillförlitlig. Kandidatvägen bevarar en möjlig passage tills ett
    // rent fix bekräftar eller motbevisar sidan av bron.
    if (GPSJumpAnalyzer.needsPassageConfirmation(vessel)) {
      this.logger.debug(`🛡️ [GPS_GATE] ${vesselId}: Blocking passage detection for ${bridgeName} - implausible position`);
      return true;
    }

    // Systerställesrundan 2026-08-23 (S1): SystemCoordinator klassar HELA
    // accept_with_caution-klassen (100-500 m skenbar förflyttning,
    // _positionUncertain) som level 'moderate' med protection - en nivå som
    // varken _hasActiveGPSCoordination eller gate-grenen nedan såg. En
    // position appen SJÄLV just dömt osäker kunde därför bära en målbro-
    // eller mellanbropassage rakt genom trajektoriedetekteringen (konfidens
    // 0,92) utan en enda GPS_GATE-rad: falsk passage, brotext som ljuger och
    // en ÄKTA passage som sedan aldrig bokförs. Asymmetrin var känd i koden -
    // kandidatkonsumtionen, re-cross-beviset och rörelsebeviset gatar alla på
    // BÅDA flaggorna. Behåll även koordinationsskyddet för stillaliggare
    // när det inte längre finns någon orimlighetsflagga på själva fixet.
    if (this._blocksUncertainPositionOnStillVessel(vessel)) {
      this.logger.debug(`🛡️ [GPS_GATE] ${vesselId}: Blocking passage detection for ${bridgeName} - uncertain position on stationary vessel`);
      return true;
    }
  ...
  registerCandidatePassage(vesselId, bridgeName, passageResult, vessel) {
    ...
    const candidateData = {
      ...
      vesselState: {
        ...
        // Etapp 0: fixtid/källa i snapshotten så bekräftelsens fysikfönster
        // kan mäta äkta fixseparation (se _isVesselStable).
        fixTs: vessel.fixTs,
        fixFeed: vessel.fixFeed,
        // Granskningsrunda 2 (2026-08-03): MOTTAGNINGSTIDEN måste med.
        // Korskälle-fixseparationen får bara VIDGA mottagningsfönstret
        // (GPSJumpAnalyzer.fixDtMs), och utan den här stämpeln saknas
        // referensen ⇒ regeln faller tillbaka på mottagningsålder.
        timestamp: Number.isFinite(vessel.timestamp) ? vessel.timestamp : Date.now(),
      },
    };
  ...
  confirmStableCandidates(vesselId, vessel) {
    ...
    for (const candidate of candidates) {
      ...
      // R2 2026-07-11 (GR2-1/DIVR2-1 — 2 oberoende): TTL-gränsen gäller
      // ÄVEN stable-grenen. C1:s lata spegel täckte bara instabil-grenen —
      // en kandidat äldre än 20 min BEKRÄFTADES i test/replay (där prod-
      // cleanupens 10s-intervall aldrig startas) men var raderad i prod:
      // test/prod-divergens av exakt C1-klassen, med risk att replay-facit
      // låser in notiser produktionen aldrig ger.
      if (age >= this._candidateTtl) {
      ...
      if (age >= this._confirmationPeriod) {
        // Etapp 0: äkta fixseparation skickas som egen parameter — age
        // (mottagningsålder) styr OFÖRÄNDRAT TTL:n ovan och bekräftelse-
        // perioden (domäninvarianten). V8 (2026-08-03): regeln bor numera i
        // GPSJumpAnalyzer.fixDtMs och gäller ÄVEN korskällepar — en kandidat
        // registrerad på en aisstream-fix och bekräftad på en AISHub-fix (var
        // annat par i 'both'-läget) fick förut mottagningsåldern, som bär
        // pollfördröjningen och krymper fysikfönstret så en äkta passage
        // övergavs som "instabil".
        const fixElapsedMs = GPSJumpAnalyzer.fixDtMs(vessel, candidate.vesselState);
        const isStable = this._isVesselStable(candidate.vesselState, vessel, age, fixElapsedMs);
        ...
        if (isStable) {
          ...
          // en senare stabil tick kan bekräfta den; TTL-gränsen vaktas nu
          // uppe i loopens huvud (GR2-1) för BÅDA grenarna.
  ...
  /**
   * Etapp 3 (V1-m6): kadensmedvetet gate-fönster. Med en pollande källa
   * aktiv (batch var 65:e sekund) har 30s-gaten ALLTID löpt ut när nästa
   * poll landar — blockeringen vore en no-op och F5:s källbytesskydd
   * tandlöst. Fönstret sätts till max(bas, 1.5 × pollintervall) när AISHub
   * är aktiv och återställs till basen annars.
   * @param {number|null} pollIntervalMs - aktivt pollintervall, eller null
   *        för att återgå till basfönstret (ren websocket-drift)
   */
  setPollCadenceMs(pollIntervalMs) {
    const base = GPSJumpGateService.BASE_GATE_TIMEOUT_MS;
    const next = Number.isFinite(pollIntervalMs) && pollIntervalMs > 0
      ? Math.max(base, Math.round(1.5 * pollIntervalMs))
      : base;
    if (next !== this._gateTimeout) {
      this.logger.log(`🛡️ [GPS_GATE] Gate-fönster ${this._gateTimeout / 1000}s → ${next / 1000}s (pollkadens ${pollIntervalMs ? `${pollIntervalMs / 1000}s` : 'av'})`);
      this._gateTimeout = next;
    }
  }
  ...
  /**
   * Osäker position (koordinationsnivå 'moderate') på ett STILLALIGGANDE
   * fartyg - systerställesrundan 2026-08-23 (S1).
   *
   * TRÖSKELN ÄR INGEN NY KONSTANT: den är
   * MOORING_DETECTION.MOVEMENT_PROOF_SOG_KN (0,5 kn), projektets enda
   * definition av "bevisad rörelse" (samma tröskel som förtöjningslagren,
   * kajavgången och rörelsebeviset använder). Under den har fartyget per
   * projektets egen doktrin INTE rört sig, och en samtidig skenbar
   * förflyttning på 100-500 m kan då bara vara GPS-brus - aldrig en passage.
   *
   * Kravet på FINIT sog är avsiktligt och gör varianten snäv: rörliga båtars
   * 100-500 m-segment lämnas orörda, och den okända-fart-klassen (sog
   * null/NaN, som GPSJumpAnalyzer redan behandlar mildare med sitt 5-knops
   * golv) rörs inte heller.
   *
   * BLOCKERING ÄR INTE FÖRLORAD PASSAGE: båda anropsställena
   * (VesselDataService._hasPassedTargetBridge respektive _hasPassedBridge)
   * registrerar då passagen som KANDIDAT, och tvåstegsbekräftelsen
   * (fysikfönster, ruttordningsvalidering, sidokontrakt) applicerar den om
   * den var äkta.
   * @private
   */
  _blocksUncertainPositionOnStillVessel(vessel) {
    if (!vessel) return false;

    // Fartkravet först: utan finit sog UNDER rörelsebeviströskeln gäller
    // regeln inte alls, oavsett koordinationsnivå.
    if (!Number.isFinite(vessel.sog)
        || vessel.sog >= MOORING_DETECTION.MOVEMENT_PROOF_SOG_KN) {
      return false;
    }
    ...
    // Systerställe: samma fallback som _hasActiveGPSCoordination använder när
    // ingen SystemCoordinator finns (eller mmsi saknas). Nivån 'moderate'
    // skrivs till fältet enbart av SystemCoordinator._handleUncertainPosition,
    // som ALLTID sätter protection - nivån ensam räcker därför här.
    return vessel.lastCoordinationLevel === 'moderate';
  }
   ...
   * @private
   */
  _isVesselStable(oldState, newState, elapsedMs = null, fixElapsedMs = null) {
    ...
    // bekräftelsen (S-F1).
    const oldSog = GPSJumpAnalyzer.positionSog(oldState);
    const newSog = GPSJumpAnalyzer.positionSog(newState);
    ...
    // Etapp 0 + V8: fysikfönstret mäter FIXSEPARATION när den finns (samma
    // källa, eller korskälla med framåt separation — se
    // GPSJumpAnalyzer.fixDtMs) — en pollande källas batch kan bära ett fix
    // som är äldre än mottagningen, och mottagningsåldern ger då fel fönster.
    // age/elapsedMs styr oförändrat TTL + bekräftelseperiod.
    const physicsElapsedMs = (Number.isFinite(fixElapsedMs) && fixElapsedMs > 0)
      ? fixElapsedMs
      : elapsedMs;
    ...
    if (Number.isFinite(physicsElapsedMs) && physicsElapsedMs > 0) {
      ...
      const elapsedHours = physicsElapsedMs / 3600000;
