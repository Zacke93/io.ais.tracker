...
const { APPROACHING_RADIUS, UNDER_BRIDGE_SET_DISTANCE } = require('../constants');
...
class CurrentBridgeManager {
  ...
  updateCurrentBridge(vessel, proximityData) {
    ...
    // R2 2026-07-11 (BR2-1): samma passedBridges-villkor som Regel 1 (B-1) —
    // utan det rev Regel 0 varje tick det Regel 1 nyss beviljade på U-svängens
    // returben (reversalen rensar listan men inte lastPassedBridge) →
    // sätt/rensa-oscillation genom hela 50–500 m-bandet, empiriskt
    // reproducerad, med waiting-hysteres-kollaps och ETA-sågtand som följd.
    // J9 (helkodsgranskning runda 2, 2026-08-22): den nakna 50:an var samma tal
    // som under-bro-hysteresens SET-gräns men bunden till ingenting — och
    // StatusServices S-3-spärr mäter i BANDET mellan denna gräns och
    // UNDER_BRIDGE_CLEAR_DISTANCE (70 m). Glider de två isär uppstår ett band
    // där currentBridge är rensad medan under-bro-låset fortfarande gäller, och
    // spärrens avståndsmätning tappar fotfästet. Bind talet så banden inte kan
    // glida isär oavsiktligt; värdet är oförändrat 50 m ⇒ noll beteendediff.
    if (vessel.currentBridge
        ...
        && (!Array.isArray(vessel.passedBridges)
          || vessel.passedBridges.includes(vessel.currentBridge))) {
      this.logger.debug(
    ...
    // J9: samma bindning som Regel 0 ovan — spärrbandets nedre gräns.
    const nearestIsRecentlyPassed = nearest
      && nearest.name === vessel.lastPassedBridge
      && nearest.distance > UNDER_BRIDGE_SET_DISTANCE
      && (!Array.isArray(vessel.passedBridges)
