...
class ProgressiveETACalculator {
  constructor(logger, bridgeRegistry) {
    ...
    // Fältprov 6 (FP6-1, 2026-07-11, KNIGHT OWL): mmsi vars ETA-historik
    // just rensats av en TARGET-TRANSITION. Rensningen tar även fartbufferten,
    // så nästa beräkning för en nära-stillastående båt saknar ALL dämpning
    // (outlier-skyddet ger no_history, EMA saknar baslinje) och 0,5 kn-golvet
    // publiceras rått — KNIGHT OWL fick "om 57 minuter" i bridge_text medan
    // hon låg och förtöjde 354 m norr om Klaffbron. Holden håller ETA på
    // null (⇒ "ETA okänd") tills båten bevisligen rör sig igen.
  ...
  _getEffectiveSpeed(vessel) {
    ...
    // FP6-1 (2026-07-11, KNIGHT OWL): post-transition stationär-hold.
    // Direkt efter en target-transition är BÅDE ETA-historiken och fart-
    // bufferten rensade — outlier-skyddet svarar no_history och EMA:n saknar
    // baslinje, så 0,5 kn-golvet på en stillastående båt publiceras helt
    // odämpat (882 m ⇒ "om 57 minuter" för en båt som förtöjer). Kontrasten
    // i samma körning: med historik kvar kapade outlier-skyddet ett identiskt
    // golvfabrikat 23,9 → 3,8 min. Så länge båten är stationär (≤ 0,3 kn,
    // samma tröskel som SPEED_FALLBACK-loggen) efter målbytet är det ärliga
    // svaret ETA okänd; första provet med verklig fart släpper holden och
    // numerisk ETA återupptas med ny baslinje. SPIKEN/AIR-klassen (stilla-
    // liggande med LÅNGLIVAT mål och intakt historik — golden-låst korrekt
    // beteende) berörs inte: holden armeras enbart av target_transition.
    if (this._postTransitionStationaryHold.has(String(vessel.mmsi))) {
      if (actualSpeed <= 0.3) {
        this.logger.debug(
          ...
          + 'right after target transition — no damping baseline exists, returning unknown ETA instead of floor-fabricated value',
        );
        return null;
      }
  ...
  clearVesselETAHistory(mmsi, reason = 'unknown') {
    ...
    // FP6-1: ett äkta BRO-TILL-BRO-målbyte armerar stationär-holden — se
    // _getEffectiveSpeed. Två vägar rensar historiken vid samma transition
    // (VDS-transitionen 'target_transition_X_to_Y' och app.js STEG 1
    // 'target_bridge_change_X_to_Y' på eventet — den senare körs SIST, så
    // båda måste armera; annars raderade dubbelclearen holden innan ETA-
    // beräkningen hann köra). Första tilldelningen ('..._none_to_X') och
    // målsläpp ('..._to_none') armerar INTE — en stillaliggare som får sitt
    // FÖRSTA mål är SPIKEN/AIR-klassen vars långa ETA är golden-låst korrekt.
    // Övriga rensningsskäl (removal, stale target) städar holden.
    const holdKey = String(mmsi);
    const isBridgeToBridgeChange = typeof reason === 'string'
