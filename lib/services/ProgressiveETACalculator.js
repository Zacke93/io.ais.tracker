...
class ProgressiveETACalculator {
  ...
  calculateProgressiveETA(vessel, proximityData) {
    try {
      ...
      // FP7-2 PRÖVAD OCH ÅTERTAGEN (2026-07-12, ALICE): en GEOMETRISK
      // bakbensvakt ("nearest akter om + riktningslås ⇒ direktberäkning",
      // utan Fix 3b:s bokföringskrav) stängde ALICE:s ETA-uppåtstuds
      // (12→14→15→16 min medan hon närmade sig Klaffbron — Olidebron-
      // korsningen felklassades som kö-drift när inträdessamplet landade
      // 6 m från brolinjen, så lastPassedBridge sattes aldrig och Fix 3b
      // missade). MEN vakten fällde 5 låsta korpusgoldens + FATAL
      // ETA-OSCILLATION 2→6→2 (14h): metodvalet flappade när nearest-
      // relationen växlade, och korpusarnas låsta progressive-serier
      // innehåller många legitima nearest-bakom-lägen. ALICE-klassen
      // (obokförd nyss-passerad mellanbro ⇒ stigande rå-ETA) accepteras
      // som känd, självläkande och redan dämpad av ETA_APPROACH_LIMIT
      // (+1 min/cykel) + EMA — dokumenterad i docs/faltprov7-2026-07-12.md.
