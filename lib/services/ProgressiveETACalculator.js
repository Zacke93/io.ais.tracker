...
class ProgressiveETACalculator {
  ...
  _getEffectiveSpeed(vessel) {
    ...
    // en STATIONÄR båt (≤ 0,3 kn, SPEED_FALLBACK-tröskeln) UTAN dämpnings-
    // baslinje får ingen numerisk ETA. Utan historik svarar outlier-skyddet
    ...
    // FP6-1/FP7-1: armerad hold + stationär (≤ 0,3 kn, SPEED_FALLBACK-
    // tröskeln) ⇒ ETA okänd; första provet med verklig fart släpper holden
    ...
    if (this._postTransitionStationaryHold.has(String(vessel.mmsi))) {
      if (actualSpeed <= 0.3) {
        this.logger.debug(
          `🧮 [ETA_POST_TRANSITION_HOLD] ${vessel.mmsi}: stationary (${actualSpeed.toFixed(1)}kn) `
          + 'after target transition/hard-stale — no damping baseline exists, returning unknown ETA instead of floor-fabricated value',
        );
        return null;
      }
