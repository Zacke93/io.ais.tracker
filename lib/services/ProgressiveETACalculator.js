...
class ProgressiveETACalculator {
  ...
  _getEffectiveSpeed(vessel) {
    ...
    // samples < 1.0kn. This prevents a single 0kn reading from a maneuvering
    // vessel (entering a lock, brief current stop) from dropping the floor —
    // that caused false flips to "inväntar broöppning" in narrow canals.
