...
class ProgressiveETACalculator {
  ...
  _getEffectiveSpeed(vessel) {
    ...
    // After a passage we expect the vessel to accelerate again; keep ETA realistic by
    // flooring the speed so we don't display 30–50 minute ETAs for short legs.
    const minRouteSpeed = hasRecentPassageContext ? MIN_PASSAGE_ROUTE_SPEED_KNOTS : 0.5;
