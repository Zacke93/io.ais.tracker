...
class ProgressiveETACalculator {
  ...
  _getEffectiveSpeed(vessel) {
    ...
    const hasRecentPassageContext = vessel.status === 'passed'
      || Boolean(vessel.lastPassedBridge)
      || vessel.pendingPassedAnnouncement
      || vessel.pendingTargetBridge
      || vessel.pendingPassed;

    // Bug #3 fix: the passage speed-floor (2.5kn) exists to avoid absurdly long
    // ETA on short legs when a vessel briefly slows while passing a bridge.
    // It MUST NOT be applied to vessels that are genuinely stationary
    // (SOG averaged below 0.8kn), otherwise ghost vessels anchored near a
    // bridge produce a deterministic ETA of ~15 minutes every minute, forever.
    // Threshold 0.8kn chosen above GPS noise floor (~0.3kn) and well below
    // normal transit speed (3–8kn) so maneuvering vessels still benefit.
    const vesselIsMoving = avgSpeed > 0.8;
  ...
  _processETAWithProtection(vessel, rawETA, proximityData) {
    ...
    if (history.length > 0) {
      ...
      // Bug #6 fix: the percent-based outlier detector fails to catch large
      // ETA swings when the vessel is nearly stationary — e.g. observed
      // "64min → 82min → 106min → 80min → 49min" within 2 minutes.
      // Add an absolute cap: consecutive ETA readings may not jump by more
      // than 10min per sample when the interval is < 2 minutes. Clamp toward
      // the previous value by allowing ±3min of real change.
      const absoluteJump = Math.abs(protectedETA - lastEntry.processedETA);
      if (absoluteJump > 10 && timeDelta < 120000) {
        const direction = Math.sign(protectedETA - lastEntry.processedETA);
        const clampedETA = lastEntry.processedETA + direction * 3;
        this.logger.debug(
          `🛡️ [ETA_ABSOLUTE_CLAMP] ${mmsi}: |ΔETA|=${absoluteJump.toFixed(1)}min `
          + `in ${(timeDelta / 1000).toFixed(1)}s → clamping ${protectedETA.toFixed(1)}min → ${clampedETA.toFixed(1)}min`,
        );
      ...
      if (vessel.sog > 2 && distanceToTarget !== null && lastEntry.distanceToTarget !== null
        ...
        if (etaIncrease > 0) {
          ...
          this.logger.debug(`🛡️ [ETA_APPROACH_LIMIT] ${mmsi}: Clamping ETA (no increase for approaching vessel, was +${etaIncrease.toFixed(1)}min)`);
        }
      }
    }
