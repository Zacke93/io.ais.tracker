...
class ProximityService extends EventEmitter {
  ...
  isApproaching(vessel, bridgeId) {
    ...
    const distance = geometry.calculateDistance(vessel.lat, vessel.lon, bridge.lat, bridge.lon);
    // B5-fix: null <= X är true i JS — kräv ett finit avstånd
    const isWithinRadius = Number.isFinite(distance) && distance <= APPROACHING_RADIUS; // FIX: Use 500m for approaching, not 300m
