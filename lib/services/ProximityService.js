...
class ProximityService extends EventEmitter {
  constructor(bridgeRegistry, logger) {
    ...
    this.logger = logger;
    // Samma mönster som VesselDataService: i produktion ÄR loggern
    // app-instansen (app.js: `new ProximityService(this.bridgeRegistry, this)`),
    // så app-lagrets självlärande kajkarta går att nå härifrån. I enhetstester
    // är loggern en mock utan metoderna — därför prövas de per anrop.
  ...
  // Fable-granskningen 2026-08-10 (FG-D2): findNearestBridge-wrappern raderad —
  // noll anropare i hela repot (inkl. tester, som testar geometrimodulens
  // funktion direkt). Anropa geometry.findNearestBridge(vessel, bridges).
  ...
  calculateProximityTimeout(vessel, proximityResult) {
    ...
    // J17 (helkodsgranskning runda 2, 2026-08-22) — LÄS DETTA INNAN DU "FIXAR"
    // GRENEN. Den gamla rubriken ("must keep vessel for at least 60 seconds")
    // ÄR FALSK, felet är mätt, och grenen står kvar ETT MEDVETET BESLUT.
    //
    // VAD GRENEN FAKTISKT GÖR: den är ett TAK på 65 s, inte ett golv.
    // remainingTime = 60000 − timeSincePassed + 5000 = 65000 − timeSincePassed,
    // och grenen nås bara när 0 ≤ timeSincePassed < 60000 ⇒ remainingTime ≤
    // 65000. Math.max(remainingTime, 65000) är därför KONSTANT 65 000 ms —
    // debugraden nedan skriver ut remainingTime, alltså INTE det returnerade
    // värdet. Och eftersom returen sker HÄR, före närhetsklasserna och före
    // golven för waiting / fast / ACTIVE_JOURNEY_MIN / moored, ÖVERSKRIVER
    // talet dem i stället för att golva dem: samma fartyg mäts till 65 s som
    // `passed` och till 1 800 000 ms som `en-route` — i den enda minut där
    // nästa målbro sätts.
    //
    // KÄNT FEL SOM ALLTSÅ STÅR KVAR: scheduleCleanup bokför ANROPARENS värde
    // som BASNIVÅ (K18: `const baseTimeoutMs = timeout`), alltså 65 000 ms —
    // exakt AISHUB_POLL_INTERVAL_MS och under den kadens fältet mäter (65–70 s
    // med jitter). noteVesselSeen laddar om max(kvarvarande, BAS), så ett
    // livstecken kan per konstruktion inte bära fartyget över ett pollglapp:
    // en båt som passerar och tystnar i just den minuten kan dö mitt i aktiv
    // resa och återfödas som ny (journey-state nollad), tvärtemot RC8:s
    // 30-minuterslöfte. Det VISNINGSFÖNSTER rubriken tror sig skydda
    // (PASSAGE_TIMING.PASSED_HOLD_MS, 150 s) skyddas i själva verket längre ned
    // i kedjan av PASSAGE_PROTECTION i scheduleCleanup (Math.max(timeout,
    // timeRemaining + 5 s, 60 s), aldrig förkortande) — det är TIMERN som
    // förlängs där, medan basnivån förblir de 65 s som returneras här.
    //
    // VARFÖR TAKET ÄNDÅ STÅR KVAR: det är i dag den mekanism som håller SR2-3
    // (låst invariant 2026-07-11) — en båt som passerar en bro, stannar och
    // tystnar får inte spöka vidare i brotexten. Fixrunda 2 flyttade samma tal
    // sist som ett äkta golv, och mätningen på låst korpus gav då:
    // 20260707-14h idx 25 gick från facit "Tre båtar … strax" (08:43:41.910)
    // till "Fyra båtar på väg mot Stridsbergsbron, ETA okänd". Ägare AKIRA
    // 257605080: kajliggare 0,1 kn, 409 m norr om den REDAN passerade
    // Klaffbron, sista sampel 08:37:10.640, finns inte i gt-passages. Hon levde
    // 6,5 min med taket och 25 min utan det — exakt den F4-I-spökbåtsklass
    // corpora.js egen not säger blev BORTTAGEN 2026-07-11. Samma klass är låst
    // i 20260804-17h (MISTY). Taket är alltså oavsiktligt, men effekten är
    // facitlåst: en ren radflytt ÄR en sanningsregression i pelare 1.
    //
    // KRAV PÅ EN FRAMTIDA FIX (kirurgi, inte flytt av raden):
    //  1) Det måste vara en KLASSREGEL för PASSERAT-OCH-STANNAT som täcker
    //     BÅDA storheterna — retention (denna timeout) OCH staleDisplayLimit
    //     i VesselDataService. Bara den ena räcker inte: taket håller i dag
    //     både registret och texten, så om det tas bort måste båda ersättas.
    //  2) C11b/U1-vakten på nearStationary-grenen räcker BEVISAT INTE: grenen
    //     stängdes av helt i en mätkopia och 20260707-14h förblev röd med
    //     IDENTISK diff, eftersom KÖ-klassen (sog < 3,0 + närmaste OPASSERADE
    //     bro inom 600 m) ger samma 25 min — AKIRA har Järnvägsbron på 568 m.
    //  3) Fixen måste MÄTAS mot 20260707-14h och 20260804-17h innan något facit
    //     rörs, och SR2-3 måste hålla i båda.
    //  4) Räkna med följdrörelser: radflytten är LASTBÄRANDE för golden-text.
    //     Fixrunda 2 låste om SJU goldens med den inne; när den togs bort igen
    //     (2c) gick alla sju tillbaka till HEAD-facit, exakt och utan rester —
    //     avvikelserna var 5 ms–30 s publiceringsskift plus en post till/från
    //     i 20260702-2h och 20260804-17h. Varje framtida ändring av kvarhåll-
    //     ningen flyttar alltså publiceringstidpunkter i HALVA korpusbanken.
    //
    // Dagens beteende är låst av karakteriseringstestet
    // tests/j17-passage-taket-ar-inte-ett-golv.test.js.
    //
    // L37 (helkodsgranskning runda 3, 2026-08-22) — PEKAREN VAR FALSK ÄNDA
    // TILLS NU: raden hänvisade till "j17-passage-golv-inte-tak.test.js"
    // (skrivet utan sökväg här just för att inte se ut som en levande pekare),
    // ett filnamn som ALDRIG har funnits i git — 2c döpte filen till namnet
    // ovan när fixen återkallades, och det namnet beskriver numera innehållet
    // korrekt. Felet stod dessutom DUBBLERAT: testfilens egen header upprepade
    // påståendet att den låste motsatsen till sitt namn.
    //
    // VARFÖR DET ÄR VÄRT SEX RADER: en falsk testpekare är exakt den klass av
    // fel som gav AKIRA-regressionen i fixrunda 2b. Den som grepar efter
    // namnet får noll träffar, drar slutsatsen att låset saknas — och tar bort
    // just den rad låset skyddar. Flyttas eller mjukas grenen upp SKA testet
    // ovan falla; hittas inte testet är det pekaren som är fel, inte låset.
    if (vessel.status === 'passed') {
      ...
      if (timeSincePassed < 60000) {
        const remainingTime = 60000 - timeSincePassed + 5000; // = 65000 − timeSincePassed
        this.logger.debug(
          `⏱️ [PASSED_TIMEOUT] ${vessel.mmsi}: Keeping for ${(remainingTime / 1000).toFixed(0)}s more (passed ${(timeSincePassed / 1000).toFixed(0)}s ago)`,
        );
        return Math.max(remainingTime, 65000); // KONSTANT 65 s — se härledningen ovan
      }
    }
    ...
    // BX-1/BX-4 (söndagsfältet 2026-08-09): FÖRTÖJNINGSGRENEN.
    // Filen innehöll NOLL förekomster av _moored, och den inlärda kajkartan
    // konsulterades aldrig i livslängdsbeslutet — trots att appen i samma
    // millisekund kunde ha loggat ⚓ [MOORED] för samma fartyg (265552100:
    // MOORED 09:33:41.466, PROXIMITY_TIMEOUT 2.0min 09:33:41.472, raderad
    // 09:35:41.473). En kajliggare >600 m från bro fick alltså FAR_DISTANCE
    // (120 s) medan hennes transponder sänder var 180:e sekund — raderingen
    // var därmed en följd av kontraktet, inte av tystnad.
    //
    // TVÅ oberoende kunskapskällor, båda redan i appen:
    //  1) vessel._moored — förtöjningsdetekteringens femlagersklassning.
    //  2) den självlärande kajkartan (F4-L) — platser där båtar BEVISLIGEN
    //     legat förtöjda. Den fångar fartyg vars klassning ännu inte hunnit
    //     fyra (navstatus 15/null + utanför de statiska kapslarna, dvs. exakt
    //     ELFKUNGEN-klassen) och läses med samma 100 m-radie som appens
    //     övriga konsumenter.
    // Grenen kan bara FÖRLÄNGA (Math.max) — den kan aldrig korta ett liv.
    const mooredLike = vessel._moored === true || this._isOnLearnedQuay(vessel);
    if (mooredLike) {
      baseTimeout = Math.max(baseTimeout, TIMEOUT_SETTINGS.MOORED_VESSEL_MIN);
    }

    // K32(j)-slaktet på LIB-sidan (fältprov 10, 2026-08-19): raden skrev
    // `status=en-route` ensamt, medan appen 6 ms tidigare kunde ha loggat
    // ⚓ [MOORED] för samma fartyg (fältfallet 265552100, 09:33:41.466 mot
    // .472). Raden såg då ut att MOTSÄGA förtöjningsklassningen. Det är två
    // olika axlar: `status` är ruttstatusen från StatusService (som saknar
    // 'moored' som värde — förtöjda fartyg landar i default-grenen
    // 'en-route'), medan `mooredLike` är förtöjningsflaggan + den inlärda
    // kajkartan, och det är DEN som just förlängde timeouten här ovan.
    // Båda axlarna skrivs nu ut, så raden förklarar sitt eget utfall.
    // Endast sträng: `mooredLike` är redan beräknad ovan och läses bara.
    this.logger.debug(
      `⏱️ [PROXIMITY_TIMEOUT] ${vessel.mmsi}: distance=${Number.isFinite(distance) ? distance.toFixed(0) : 'unknown'}m, `
      + `speed=${vessel.sog}kn, status=${vessel.status} (ruttstatus), mooredLike=${mooredLike} `
      + `(förtöjd/inlärd kaj), timeout=${(baseTimeout / 60000).toFixed(1)}min`,
    );
  ...
  /**
   * BX-4: ligger fartyget på en INLÄRD kajplats? Ren läsning av app-lagrets
   * kajkarta — ProximityService äger ingen egen kopia (en parallell sanning
   * om var kajerna ligger vore värre än ingen alls).
   * @private
   * @param {Object} vessel
   * @returns {boolean}
   */
  _isOnLearnedQuay(vessel) {
    if (!this.app || typeof this.app._isNearLearnedMooringSpot !== 'function') return false;
    if (!Number.isFinite(vessel.lat) || !Number.isFinite(vessel.lon)) return false;
    // "Ligger på kaj" ⇒ inte i transit. Samma tröskel och samma semantik som
    // app._noteQuayStability använder för samma karta: sog=null är OKÄNT
    // (S-F7) och diskvalificerar inte — en fartgivarlös kajliggare är exakt
    // den klass grenen finns för — men en båt i fart är på genomresa och ska
    // ha sin vanliga avståndsbaserade livslängd.
    const sog = Number.isFinite(vessel.sog) ? vessel.sog : null;
    if (sog !== null && sog >= QUAY_DEPARTURE_GATE.TRANSIT_SOG_KN) return false;
    try {
      return this.app._isNearLearnedMooringSpot(vessel.lat, vessel.lon, 100) === true;
    } catch (err) {
      // Kajkartan får aldrig kunna fälla livslängdsberäkningen.
      this.logger.debug(`⏱️ [TIMEOUT_CALC] ${vessel.mmsi}: kajkartan otillgänglig (${err.message})`);
      return false;
    }
  }
