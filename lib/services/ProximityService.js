...
class ProximityService extends EventEmitter {
  ...
  calculateProximityTimeout(vessel, proximityResult) {
    ...
    // J17: den korta basnivån behålls för passerade båtar utan färska
    // transitbevis. Den begränsar bl.a. AKIRA/MISTY som stannar och tystnar.
    // scheduleCleanup förkortar ALDRIG en redan beviljad timer och skyddar
    // dessutom passagevisningen. Ett vanligt 65–70 s pollglapp raderar alltså
    // inte en pågående resa, trots den korta basnivån här.
    //
    // Däremot förnyades inte 30-minuterslöftet vid en ny passage: Olidebron
    // efter ett 19-minuters AIS-glapp lämnade bara 11 minuter av föregående
    // timer. Båten raderades före nästa fix 12 minuter senare, med Klaffbron
    // framför sig och färska rörelsebevis. Bara denna bevisade fortsättning
    // får därför de ordinarie golven. Låg/okänd råfart, GPS-osäkerhet och
    // mål bakom båten får inte förlänga passerat-och-stannat-klassen.
    // Helkedjan och båda fältstoppen prövas i
    // tests/j17-continuing-passage-retention.test.js. Den äldre enkla
    // passed-klassen låses fortfarande av
    ...
    if (vessel.status === 'passed') {
      ...
      if (timeSincePassed < 60000 && !this._hasContinuingPassageJourney(vessel)) {
        const remainingTime = 60000 - timeSincePassed + 5000; // = 65000 − timeSincePassed
        const passageTimeout = Math.max(remainingTime, 65000);
        this.logger.debug(
          `⏱️ [PASSED_TIMEOUT] ${vessel.mmsi}: Basnivå ${(passageTimeout / 1000).toFixed(0)}s (passerade för ${(timeSincePassed / 1000).toFixed(0)}s sedan)`,
        );
        return passageTimeout;
      }
    }
  ...
  /**
   * Förnyar en bokförd passage också en pågående resa? Historisk fart eller
   * klistrande rörelsebevis räcker inte: senaste råfixen måste visa transit
   * och ett rent positionssegment utanför appens befintliga jittergräns.
   * Anropas i den nya AIS-fixens analys; timeromvärderingen använder inte
   * timeoutberäkningen. Båda råfälten byggs om i _createVesselObject.
   * @private
   */
  _hasContinuingPassageJourney(vessel) {
    const passed = Array.isArray(vessel.passedBridges) ? vessel.passedBridges : [];
    if (!vessel.targetBridge || !passed.includes(vessel.lastPassedBridge)
        || passed.includes(vessel.targetBridge) || vessel._moored === true
        || vessel._gpsJumpDetected === true || vessel._positionUncertain === true
        || !Number.isFinite(vessel._rawPositionSog)
        || vessel._rawPositionSog < QUAY_DEPARTURE_GATE.TRANSIT_SOG_KN
        || vessel._positionAnalysis?.action !== 'accept'
        || !Number.isFinite(vessel._positionAnalysis.movementDistance)
        || vessel._positionAnalysis.movementDistance < MOORING_DETECTION.MOVEMENT_PROOF_NET_M) return false;

    const target = this.bridgeRegistry.getBridgeByName(vessel.targetBridge);
    const previous = this.bridgeRegistry.getBridgeByName(vessel.lastPassedBridge);
    if (!target || !previous || !Number.isFinite(vessel.lat)) return false;
    // Kanalens broar är latitudordnade; samma framför/bakom-villkor som
    // passageordningen och brotextens tidsgränser använder.
    if (vessel._routeDirection === 'north') return previous.lat < vessel.lat && vessel.lat < target.lat;
    if (vessel._routeDirection === 'south') return previous.lat > vessel.lat && vessel.lat > target.lat;
    return false;
  }
