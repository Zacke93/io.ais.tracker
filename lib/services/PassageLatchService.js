...
const { isNorthCog, isSouthCogWide } = require('../utils/cogDirection');
...
class PassageLatchService {
   ...
   *
   * J18 (helkodsgranskning runda 2, 2026-08-22) — LATCHEN ÄGER SIN EGEN
   * RIKTNINGSHÄRLEDNING. Riktningen SKREVS med anroparens band men LÄSTES
   * med latchens eget: alla fyra anropsställen i VesselDataService avslutar
   * sin fallbackkedja med _safeDetermineDirection, som använder det STRIKTA
   * sydbandet (isSouthCogStrict, 135–225) och därför ger null för COG
   * 226–314 — enligt cogDirection.js NORMAL sydfärd i den NE-SV-orienterade
   * kanalen. Läsningen i shouldBlockStatus sker med _directionFromCog och det
   * BREDA bandet (isSouthCogWide, 135–<315). Asymmetrin gjorde F13-släppet
   * dött för hela SV-klassen: en mållös båt (ingen målbro ⇒ inga ruttlås) som
   * passerar en mellanbro med COG 250 och saknar korsningsbevis lagrades med
   * direction null, och reversal-grenen kräver truthy latchData.direction ⇒
   * "inväntar"/"närmar sig" blockerades hela 10-minutersfönstret vid
   * vändningen tillbaka — och därmed även boat_near.
   *
   * Fjärde parametern är VALFRI och används bara när anroparen inte hade
   * någon riktning alls. Saknas BÅDA förblir direction null, så det
   * konservativa kontraktet ("osäker riktning blockerar") är oförändrat.
   *
   * @param {string} vesselId - Vessel MMSI
   * @param {string} bridgeName - Name of bridge passed
   * @param {string|null} direction - 'north' eller 'south' (anroparens bevis)
   * @param {number|null} [cog] - fartygets COG, används som SISTA utväg när
   *   anroparen inte kunde härleda någon riktning. Tolkas med latchens EGET
   *   (breda) band, samma som läsningen i shouldBlockStatus.
   */
  registerPassage(vesselId, bridgeName, direction, cog = null) {
    ...
    // J18: anroparens bevis vinner alltid — COG-härledningen är en fallback,
    // aldrig en överskrivning (ett korsningsbevis är starkare än en kurs).
    const cogDerived = direction ? null : this._directionFromCog(cog);
    const effectiveDirection = direction || cogDerived;
    ...
    // Källan skrivs ut: en fältanalys måste kunna skilja ett korsningsbevis
    // från latchens egen COG-härledning när ett släpp ifrågasätts.
    let kalla = 'okänd';
    if (direction) kalla = 'anropare';
    else if (cogDerived) kalla = 'cog';
    this.logger.debug(
      `🔒 [PASSAGE_LATCH] ${vesselId}: Registered passage of ${bridgeName} `
      + `(${effectiveDirection || 'null'}, källa=${kalla}${cogDerived ? `, cog=${cog}` : ''})`,
    );
  }
   ...
   */
  shouldBlockStatus(vesselId, bridgeName, proposedStatus, currentCog = null, sampleTs = null) {
    ...
    if (currentDirection && latchData.direction && currentDirection !== latchData.direction) {
      ...
      // Fable-granskningen 2026-07-10b (G-4): bekräftelsen kräver två OLIKA
      // POSITIONSSAMPEL (sampleTs skiljer), inte två anrop — waiting- och
      // approaching-prövningen körs i samma tick med samma cog och kunde
      // bekräfta varandra på ETT brusigt sampel. Och det gamla 2-min-
      // fönstret var kortare än Class B-kadensen (3–15 min): pending hann
      // alltid löpa ut och nollställas av nästa sample → F13-releasen var
      // DÖD för glesa sändare (returpassagens statusar blockerade hela
      // 10-min-latchen). Fönstret är nu 20 min (täcker maxkadensen; latchen
      // själv är 10 min, så äldre pending försvinner med latchen).
      const PENDING_MAX_AGE_MS = 20 * 60 * 1000;
      const sameSample = pending
      ...
      if (!confirmed) {
        if (!sameSample) {
          latchData.pendingReversal = {
            dir: currentDirection,
            time: Date.now(),
            sampleTs: Number.isFinite(sampleTs) ? sampleTs : null,
          };
        }
        this.logger.debug(
  ...
  /**
   * Härled riktning ur COG. Returnerar null vid osäker/saknad COG.
   *
   * Fable-granskningen 2026-08-10 (FG-D5): docstringen påstod "samma trösklar
   * som VesselDataService (_safeDetermineDirection)" — det är FALSKT sedan
   * GR2-6 och banden skiljer sig MEDVETET (samma riskprofilsresonemang som
   * P5-beslutet i lib/constants.js, COG_DIRECTIONS):
   *  - HÄR (latchens reversal-detektion): norr 315–45, syd BRETT 135–<315.
   *    Låg insats — ett felaktigt 'south' släpper på sin höjd en latch tidigt.
   *  - VesselDataService._safeDetermineDirection: norr 315–45, syd STRIKT
   *    135–225; 46–134 och 226–314 är tvetydiga → ingen riktning alls. Hög
   *    insats (fel målbro/route-lås) ⇒ konservativt band.
   * Konkret: cog 250 ⇒ 'south' här, men null i VesselDataService.
   *
  ...
  _directionFromCog(cog) {
    ...
    // Fable-granskningen 2026-08-10 (FG-DIR): gradliteralerna lyfta till den
    // namngivna predikatfamiljen (lib/utils/cogDirection) — banden oförändrade.
    // Att valet syns i NAMNET (Wide, inte Strict) är hela poängen: skillnaden
    // mot VesselDataService som docstringen ovan varnar för står nu i koden.
    // Normaliseringen sker kvar här; predikaten normaliserar medvetet inte.
    if (isNorthCog(normalizedCog)) return 'north';
    // R2 2026-07-11 (GR2-6): A1-1-harmoniserat sydband 135–<315 — det smala
    // 135–225 gjorde SV-kurs till 'okänd' → F13-releasen död för klassen
    // (returpassagens statusar blockerade hela 10-min-latchen).
    if (isSouthCogWide(normalizedCog)) return 'south';
    return null;
  }

  // Helgranskning 2026-07-10: shouldBlockMessage raderad — noll produktions-
  // anrop, och båda grenarna anropade shouldBlockStatus identiskt UTAN
  // currentCog (F13-reversalen hade aldrig kunnat trigga om den togs i bruk).
  // Använd shouldBlockStatus direkt med vessel.cog.
