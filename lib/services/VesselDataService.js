...
class VesselDataService extends EventEmitter {
  ...
  _shouldAssignTargetBridge(vessel) {
    ...
    if (!Number.isFinite(vessel.cog) && nearestDistance > 300) {
      this.logger.debug(
        `🚫 [TARGET_ASSIGNMENT] ${vessel.mmsi}: Invalid/missing course (${vessel.cog}) and far from bridges (${nearestDistance.toFixed(0)}m) - likely stationary/anchored`,
      );
      return false;
    ...
    if (hasPassedCurrentTarget) {
      ...
      if (targetBridge) {
        ...
        if (distanceToCurrentTarget <= 200) { // Still very close to current target bridge (increased threshold)
          ...
          // Also check if we have confirmed passage via _wasCloseToTarget
          ...
          this.logger.debug(
            `✅ [TARGET_TRANSITION_ALLOWED] ${vessel.mmsi}: Allowing transition despite ${distanceToCurrentTarget.toFixed(0)}m distance - vessel has passed ${vessel.targetBridge}`,
          );
        }
      }
  ...
  _createVesselObject(mmsi, data, oldVessel) {
    ...
    return {
      ...
      _wasCloseToTarget: oldVessel?._wasCloseToTarget || null, // Persist close-to-target tracking
    };
  }
  ...
  _hasPassedTargetBridge(vessel, oldVessel) {
    ...
    if (hasPassed) {
      ...
      // Clear the close tracking after successful passage detection
