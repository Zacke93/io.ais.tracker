...
class VesselDataService extends EventEmitter {
  ...
  getVesselsForBridgeText() {
    ...
    const filteredVessels = allVessels.filter((vessel) => {
      ...
      if (!hasTargetBridge && !isNearStallbackabron) {
        this.logger.debug(
          `❌ [BRIDGE_TEXT_FILTER] ${vessel.mmsi}: No valid targetBridge and not near Stallbackabron - excluding from bridge text `
    ...
    if (hasPassedCurrentTarget) {
      ...
      if (targetBridge) {
        ...
        if (distanceToCurrentTarget <= constants.PROTECTION_ZONE_RADIUS) { // Still within protection zone (300m)
          ...
          if (!recentlyPassed) {
            this.logger.debug(
              `🛡️ [TARGET_TRANSITION_BLOCKED] ${vessel.mmsi}: Blocking targetBridge change while ${distanceToCurrentTarget.toFixed(0)}m from ${vessel.targetBridge} (within ${constants.PROTECTION_ZONE_RADIUS}m protection zone)`,
            );
  ...
  _isVesselNearStallbackabron(vessel) {
    ...
    // CRITICAL: Only include if near Stallbackabron AND has relevant status
    const shouldInclude = (isWithinApproachingRadius || hasStallbackaStatus || isUnderStallbackabron) && hasRelevantStatus;

    if (shouldInclude) {
      this.logger.debug(
        `✅ [STALLBACKABRON_NEAR] ${vessel.mmsi}: Near Stallbackabron (${distance.toFixed(0)}m, status=${vessel.status}) - including in bridge text`,
      );
    } else if (isWithinApproachingRadius) {
      this.logger.debug(
        `❌ [STALLBACKABRON_EXCLUDED] ${vessel.mmsi}: Near Stallbackabron (${distance.toFixed(0)}m) but irrelevant status (${vessel.status})`,
      );
    }
