...
class VesselDataService extends EventEmitter {
  ...
  _updateMooringEvidence(vessel) {
    ...
    // Förtöjd-klassning (lager 3 + 4 + 5) — kräver alltid stillhet
    let moored = false;
  ...
  _findMooringZone(vessel, extraRadiusM = 0) {
    for (const zone of constants.MOORING_ZONES) {
      ...
      if (dist <= zone.radiusM + extraRadiusM) {
        return zone;
      }
    }
    return null;
  ...
  _isInProtectionZone(vessel) {
    ...
    for (const bridge of Object.values(this.bridgeRegistry.bridges)) {
      ...
      if (distance <= protectionRadius && (nearest === null || distance < nearest.distance)) {
