...
class VesselDataService extends EventEmitter {
  ...
  updateVessel(mmsi, data) {
    ...
    this._updateMooringEvidence(vessel);
    ...
    if (hasPassedCurrentTarget) {
      ...
      if (confirmedPassage && vessel.targetBridge) {
        if (this.app?.passageLatchService) {
          const latchDirection = vessel._finalTargetDirection
            || vessel._routeDirection
        ...
        if (this.app?.routeOrderValidator) {
          this.app.routeOrderValidator.registerPassage(
            vessel.mmsi.toString(), vessel.targetBridge, vessel,
          );
        }
      }
  ...
  _hasPassedTargetBridge(vessel, oldVessel) {
    ...
    if (passageResult.passed) {
      ...
      if (this.app.passageLatchService) {
        ...
        const direction = vessel._finalTargetDirection
          || vessel._routeDirection
          || this._safeDetermineDirection(vessel.cog);
        this.app.passageLatchService.registerPassage(
          vessel.mmsi.toString(),
  ...
  registerConfirmedIntermediatePassage(vessel, oldVessel, bridgeName, confirmedAt) {
    ...
    if (this._anchorPassageTimestamp(vessel, bridgeName, passageTimestamp)) {
      const passageId = this._generatePassageId(vessel.mmsi, bridgeName, vessel);
      if (!this._isPassageAlreadyProcessed(passageId)) {
        vessel.lastPassedBridgeTime = passageTimestamp;
        vessel.lastPassedBridge = bridgeName;
        this._setForceWaitingForCloseBridgePair(vessel, bridgeName);
        this._markPassageProcessed(passageId);
        ...
        if (this.app?.routeOrderValidator) {
          this.app.routeOrderValidator.registerPassage(
            vessel.mmsi.toString(), bridgeName, vessel,
          );
        }
    ...
    for (const bridge of orderedBridges) {
      ...
      if (hasPassedThisBridge) {
        ...
        if (vessel.targetBridge) {
          ...
          const crossDir = this._evidencedCrossingDirection(vessel, oldVessel, bridge);
          if (crossDir && vessel._routeDirection && crossDir !== vessel._routeDirection) {
  ...
  _setForceWaitingForCloseBridgePair(vessel, passedBridgeName) {
    ...
    if (pairedBridge) {
      ...
      const fixUDir = vessel._finalTargetDirection || vessel._routeDirection || null;
