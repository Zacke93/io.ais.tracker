...
class VesselDataService extends EventEmitter {
  ...
  updateVessel(mmsi, data) {
    ...
    if (isNewVessel && !vessel.targetBridge) {
      ...
      if (!protectionActive) {
        ...
        } else if (!this._shouldAssignTargetBridge(vessel, oldVessel) && effectiveProtection) {
  ...
  _updateMooringEvidence(vessel) {
    ...
    // Stillaliggande-klocka (lager 5-underlag). sog null = okänd → räkna
    // INTE som stillaliggande (konservativt: hellre behålla än demotera).
    const isStationary = sog !== null && sog < cfg.STATIONARY_SOG_KN;
    if (isStationary) {
    ...
    if (isStationary) {
      if (cfg.MOORED_NAV_STATUSES.includes(vessel.navStatus)) {
        moored = true;
   ...
   */
  _handleTargetBridgeTransition(vessel, oldVessel, { confirmedPassage = false } = {}) {
    if (!oldVessel) {
      return;
    }
