...
class VesselDataService extends EventEmitter {
  ...
  _isVesselSuitableForBridgeText(vessel) {
    ...
    if (vessel.status === 'waiting' || vessel.status === 'stallbacka-waiting') {
      if (!vessel.targetBridge) {
        this.logger.debug(
          `❌ [BRIDGE_TEXT_FILTER] ${vessel.mmsi}: Waiting vessel without targetBridge - excluding from bridge text`,
        );
        return false; // EXCLUDE vessels without target bridge
      }

      this.logger.debug(
        `✅ [BRIDGE_TEXT_FILTER] ${vessel.mmsi}: Waiting vessel (${vessel.status}) with target ${vessel.targetBridge} - include in bridge text`,
      );
      return true;
    }
  ...
  _handleTargetBridgeTransition(vessel, oldVessel) {
    ...
    if (vessel.lastPassedBridgeTime
      ...
      this.logger.debug(
        `⏸️ [TARGET_PROTECTION] ${vessel.mmsi}: Within 60s "precis passerat" window for SAME bridge - keeping targetBridge`,
      );
    ...
    if (hasPassedCurrentTarget) {
      ...
      if (nextTargetBridge && nextTargetBridge !== vessel.targetBridge) {
        ...
        this.logger.debug(`✅ [TARGET_REMOVED] ${vessel.mmsi}: Removed targetBridge after passing final bridge (status: ${vessel.status})`);
        vessel.lastPassedBridgeTime = Date.now();
        vessel.lastPassedBridge = oldVessel.targetBridge;
      }
    } else {
