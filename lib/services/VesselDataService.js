...
class VesselDataService extends EventEmitter {
  ...
  getVesselsForBridgeText() {
    ...
    const filteredVessels = allVessels.filter((vessel) => {
      ...
      const staleDisplayLimit = nearStationaryAtLastContact
        ? 25 * 60 * 1000
        : STALE_DISPLAY_MS;
  ...
  _updateMooringEvidence(vessel) {
    ...
    if (isStationary) {
      if (vessel._moored) {
        ...
        if (zone) {
          moored = true;
          mooredReason = `zon "${zone.name}"`;
        } else if (vessel._stationarySince
   ...
   * @returns {boolean}
   */
  isNearMooringZone(lat, lon) {
    if (!Number.isFinite(lat) || !Number.isFinite(lon)) return false;
    return this._findMooringZone({ lat, lon }) !== null;
  }
