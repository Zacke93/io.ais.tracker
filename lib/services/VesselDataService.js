...
class VesselDataService extends EventEmitter {
  ...
  removeVessel(mmsi, reason = 'timeout') {
    ...
    // Helgranskning 2026-07-06 (vds-1#R2-1): timeout-varianten krävde bara
    // NÅGON passage — en mållös båt som passerat enbart Olidebron och
    // tystnat (gap-klassen) registrerades som "avslutad resa", och
    // reentry-blocket (rad 98) IGNORERAR då all AIS i 10 min vid återkomsten
    // → missade målbronotiser mitt i en pågående resa. En resa är avslutad
    // först när en MÅLBRO (Klaffbron/Stridsbergsbron) faktiskt passerats —
    // Bug E:s äkta fall (terminalpassage följt av timeout-removal) uppfyller
    // det; halvfärdiga resor gör det inte.
    const passedATargetBridge = Array.isArray(vessel.passedBridges)
      && vessel.passedBridges.some((b) => this.bridgeRegistry.isValidTargetBridge(b));
