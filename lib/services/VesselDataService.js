...
class VesselDataService extends EventEmitter {
        ...
        if (targetBridgeObj
          ...
          if (shouldRecalc) {
            ...
            if (!pendingConfirmed) {
              ...
              this.logger.log(
                `🔄 [TARGET_RECALC] ${vessel.mmsi}: Direction reversed CONFIRMED `
              ...
              // Produktionsredo-granskningen (2026-07-03, CONFIRMED high):
              // protection måste släppas HÄR — spegla moored-vägen. Utan
              // detta RESTORE:ade _checkTargetBridgeProtection den övergivna
              // bron som target i upp till 5 min (skyddet aktiveras nästan
              // garanterat av själva U-svängen: cogChange >45° = maneuver),
              // och B3-vakten var besegrad eftersom passedBridges töms nedan.
              this.targetBridgeProtection.delete(mmsi);
              this._clearProtectionTimer(mmsi);
              // N1 (2026-07-01): en bekräftad U-sväng MITT i resan är en NY
              // resa i motsatt riktning. Utan detta blockerade passedBridges
              // all måltilldelning (returriktningens broar låg redan i listan
              // → _calculateTargetBridge returnerade null för evigt) och
              // dedup-nycklarna kvävde returpassagens notiser → båten blev
              // OSYNLIG i bridge_text och HELT onotifierad på returresan.
              // Spegla NEW_JOURNEY-resetten: nollställ passedBridges och låt
              // app-lagret rensa dedup (session + persistent) via eventet.
              if (Array.isArray(vessel.passedBridges) && vessel.passedBridges.length > 0) {
                this.logger.log(
                  `🔁 [JOURNEY_RESET] ${vessel.mmsi}: Mid-journey reversal → clearing `
                  + `passedBridges [${vessel.passedBridges.join(', ')}] + requesting dedup reset`,
                );
                vessel.passedBridges = [];
                vessel._finalTargetBridge = null;
                vessel._finalTargetDirection = null;
                this.emit('vessel:journey-reset', { mmsi: vessel.mmsi, vessel, direction: newDir });
              }
            }
          }
        }
  ...
  _calculateTargetBridge(vessel) {
    ...
    const dir = this._safeDetermineDirection(vessel.cog);
    if (dir === null) {
      this.logger.debug(
  ...
  registerConfirmedIntermediatePassage(vessel, oldVessel, bridgeName, confirmedAt) {
    ...
    if (vessel.targetBridge) {
      ...
      const dir = vessel._routeDirection;
      const beyondTarget = targetObj && bridgeObj
        ...
        && this._targetOriginSideOk(vessel, targetObj, dir);
      if (beyondTarget) {
        this.logger.log(
