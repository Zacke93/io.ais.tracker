...
class VesselDataService extends EventEmitter {
  ...
  updateVessel(mmsi, data) {
    ...
    if (isNewVessel && !vessel.targetBridge) {
      ...
      if (!this._shouldAssignTargetBridge(vessel)) {
        this.logger.debug(
          `🚫 [TARGET_REMOVAL] ${vessel.mmsi}: Removing target bridge ${vessel.targetBridge} - vessel appears to have anchored (${Number(vessel.sog)}kn)`,
        );
  ...
  getVesselsForBridgeText() {
    ...
    const filteredVessels = allVessels.filter((vessel) => {
      ...
      if (!vessel.targetBridge && !isUnderBridgeWithoutTarget && !isStallbackabraBridgeWaiting && !isWaitingAtTargetBridge) {
        // Fallback: If waiting at target bridge, set targetBridge = currentBridge
        if (isWaitingAtTargetBridge) {
          vessel.targetBridge = vessel.currentBridge;
          this.logger.debug(
            `🔧 [BRIDGE_TEXT_FALLBACK] ${vessel.mmsi}: Set targetBridge=${vessel.currentBridge} for waiting vessel`,
          );
        } else {
          this.logger.debug(`❌ [BRIDGE_TEXT_FILTER] ${vessel.mmsi}: No targetBridge and not special case`);
          return false;
        }
      }
      ...
      if (vessel.targetBridge && !this.bridgeRegistry.isValidTargetBridge(vessel.targetBridge)) {
        this.logger.debug(`❌ [BRIDGE_TEXT_FILTER] ${vessel.mmsi}: Invalid targetBridge: ${vessel.targetBridge}`);
        return false;
      }
      ...
      this.logger.debug(`✅ [BRIDGE_TEXT_FILTER] ${vessel.mmsi}: Included in bridge text (${vessel.status}, target=${vessel.targetBridge})`);
      return true;
    });
  ...
  _shouldAssignTargetBridge(vessel) {
    ...
    // - Approaching bridges (300-500m): Must have ≥0.5kn (boats slowing down for bridge)
    // - Near bridges (<300m): Must have ≥0.3kn (UPDATED: no more 0.0kn allowance - fixes ZENIT problem)
    ...
    if (nearestDistance > 500) {
      ...
      // Check if nearest bridge is a target bridge
      let nearestBridgeName = null;
      for (const bridge of Object.values(this.bridgeRegistry.bridges)) {
        const distance = geometry.calculateDistance(
          vessel.lat, vessel.lon,
          bridge.lat, bridge.lon,
      ...
      if (isNearTargetBridge) {
        ...
        this.logger.debug(
          `🎯 [TARGET_ASSIGNMENT] ${vessel.mmsi}: Near target bridge ${nearestBridgeName} - allowing 0.0kn for waiting`,
        );
      } else {
        // Near intermediate bridge: Require some movement or recent movement
        if (vessel.lastPositionChange && (Date.now() - vessel.lastPositionChange) < 60000) {
    ...
    // COURSE VALIDATION: Course should be reasonable (not 0° or undefined)
    // BUT: Allow boats near bridges to have no course only if they have reasonable speed
    if ((!vessel.cog || vessel.cog === 0 || !Number.isFinite(vessel.cog)) && nearestDistance > 300) {
      this.logger.debug(
      ...
      return false;
    } if ((!vessel.cog || vessel.cog === 0) && nearestDistance <= 300 && speed >= minSpeed) {
      this.logger.debug(
        `⚠️ [TARGET_ASSIGNMENT] ${vessel.mmsi}: No course but near bridge with movement (${nearestDistance.toFixed(0)}m, ${speed}kn) - allowing`,
      );
    }

    this.logger.debug(
      `✅ [TARGET_ASSIGNMENT] ${vessel.mmsi}: Qualifies for target bridge (${speed}kn, ${nearestDistance.toFixed(0)}m, ${vessel.cog}°)`,
    );
    return true;
  }
  ...
  _isVesselSuitableForBridgeText(vessel) {
    ...
    // - Far from bridges (>500m): Must have ≥0.5kn (slightly more lenient than target assignment)
    // - Approaching bridges (300-500m): Must have ≥0.3kn
    ...
    if (speed < minSpeed) { // Note: < instead of <= to allow exactly 0.0kn when minSpeed is 0.0
      this.logger.debug(
        `🚫 [BRIDGE_TEXT_FILTER] ${vessel.mmsi}: Too slow for bridge text (${speed}kn < ${minSpeed}kn, ${nearestDistance.toFixed(0)}m ${context})`,
      );
      return false;
    }
    ...
    if (vessel.targetBridge && speed <= 0.6) {
      ...
      if (targetBridge) {
        ...
        if (distanceToTarget > 1000) { // 1km+ from target bridge
          this.logger.debug(
            `🚫 [BRIDGE_TEXT_FILTER] ${vessel.mmsi}: Too far from target bridge ${vessel.targetBridge} with low speed (${distanceToTarget.toFixed(0)}m, ${speed}kn) - likely anchored`,
          );
          return false;
        }
      }
  ...
  _calculateTargetBridge(vessel) {
    ...
    if (isNorthbound) {
      ...
      if (vessel.lat > stridsbergsbronLat) {
        this.logger.debug(
          `🚪 [TARGET_ASSIGNMENT] ${vessel.mmsi}: Norrut, norr om Stridsbergsbron (${vessel.lat} > ${stridsbergsbronLat}) → lämnar kanalen, ingen målbro`,
        );
      ...
      this.logger.debug(
        `🎯 [TARGET_ASSIGNMENT] ${vessel.mmsi}: Norrut, mellan broarna (${vessel.lat} >= ${klaffbronLat}) → Stridsbergsbron`,
      );
    ...
    if (vessel.lat < klaffbronLat) {
      this.logger.debug(
        `🚪 [TARGET_ASSIGNMENT] ${vessel.mmsi}: Söderut, söder om Klaffbron (${vessel.lat} < ${klaffbronLat}) → lämnar kanalen, ingen målbro`,
      );
      return null; // No target bridge - vessel leaving canal
    }
    ...
    this.logger.debug(
      `🎯 [TARGET_ASSIGNMENT] ${vessel.mmsi}: Söderut, mellan broarna (${vessel.lat} <= ${stridsbergsbronLat}) → Klaffbron`,
    );
    return 'Klaffbron';
  }
  ...
  _handleTargetBridgeTransition(vessel, oldVessel) {
    ...
    if (vessel.lastPassedBridgeTime && (Date.now() - vessel.lastPassedBridgeTime) < 60000) {
      this.logger.debug(
        `⏸️ [TARGET_PROTECTION] ${vessel.mmsi}: Within 60s "precis passerat" window - keeping targetBridge`,
      );
    ...
    this.logger.debug(
      `🔍 [TARGET_DEBUG] ${vessel.mmsi}: hasPassedCurrentTarget=${hasPassedCurrentTarget}, targetBridge=${vessel.targetBridge}`,
    );

    if (hasPassedCurrentTarget) {
      // CRITICAL FIX: Don't change targetBridge if vessel is still very close to current target (≤100m)
      ...
      if (nextTargetBridge && nextTargetBridge !== vessel.targetBridge) {
        ...
        if (!isInCriticalState) {
          ...
          this.logger.debug(`✅ [TARGET_REMOVED] ${vessel.mmsi}: Safe targetBridge removal - status: ${vessel.status}`);
        } else {
          this.logger.debug(`🛡️ [TARGET_KEPT] ${vessel.mmsi}: Keeping targetBridge - critical status: ${vessel.status}`);
        }
        vessel.lastPassedBridgeTime = Date.now();
        vessel.lastPassedBridge = oldVessel.targetBridge;
      }
  ...
  _hasPassedTargetBridge(vessel, oldVessel) {
    ...
    this.logger.debug(
      `🔍 [PASSAGE_DEBUG] ${vessel.mmsi} -> ${vessel.targetBridge}: `
  ...
  _handleGPSJumpDetection(mmsi, currentPosition, previousPosition, oldVessel) {
    ...
    if (movementDistance > MOVEMENT_DETECTION.GPS_JUMP_THRESHOLD) {
      ...
      this.logger.error(
        `🚨 [GPS_JUMP] ${mmsi}: Large movement detected ${movementDistance.toFixed(
          0,
  ...
  _hasPassedBridge(vessel, oldVessel, bridge) {
    ...
    const { LINE_CROSSING_MIN_PROXIMITY_M = 150, FEATURE_LINE_CROSSING = true } = require('../constants');
    
    let hasPassedByCrossing = false;
    if (FEATURE_LINE_CROSSING) {
      hasPassedByCrossing = geometry.hasCrossedBridgeLine(prevPos, currPos, bridge, { 
      ...
      if (hasPassedByCrossing) {
        ...
        if (!isMovingAway) {
          ...
          this.logger.debug(
            `🔀 [LINE_CROSS] ${vessel.mmsi}: Crossed ${bridge.name} line `
