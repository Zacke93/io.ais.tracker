...
class VesselDataService extends EventEmitter {
  ...
  scheduleCleanup(mmsi, timeout) {
    ...
    if (vessel && this.vesselLifecycleManager.shouldEliminateVessel(vessel)) {
      ...
      // Bug #13 fix: vessel is already queued for elimination; ignore any
      // reschedule request that would extend the timer beyond 1 second.
      this.logger.debug(
  ...
  _hasPassedTargetBridge(vessel, oldVessel) {
    ...
    if (passageResult.passed) {
      ...
      // Bug #10 observability: make low-confidence passages searchable in logs.
      // When confidence ≤ 0.50 combined with near-zero SOG, the passage is a
