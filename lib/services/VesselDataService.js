  ...
  _shouldAssignTargetBridge(vessel, oldVessel = null) {
    ...
    // falsk måltext) och återfår target via ACCELERATED vid avgång.
    {
      ...
      if (entryLat !== null && oldVessel
        ...
        // Etapp 0: nordkomponenten är fysik — mät över FIXSEPARATION när
        // båda samplen bär fixtid från samma källa. Med mottagningstids-dt
        // och en pollande källa (batch var 65:e sekund med äldre fix)
        // överskattas northMps annars ~3× och grinden släpper kajvobbel.
        const bothFixTs = vessel.fixFeed
          && vessel.fixFeed === oldVessel.fixFeed
          && Number.isFinite(vessel.fixTs) && Number.isFinite(oldVessel.fixTs);
        ...
        const dtS = bothFixTs
          ? (vessel.fixTs - oldVessel.fixTs) / 1000
          : (nowTs - prevTs) / 1000;
        if (dtS > 0) {
