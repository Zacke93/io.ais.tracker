...
const { isNorthCog } = require('../utils/cogDirection');
...
// 300 m north of Stallbackabron (58.309802 N) ≈ 58.3125 N. A northbound vessel
...
// C0 2026-08-10: talet följde med brokoordinaten till konsensuspunkten
// (58.3141 hörde till den gamla, felplacerade latituden 58.31143) — se
// C0-blocket vid BRIDGES.stallbackabron i lib/constants.js. Bufferten är
// oförändrad 300 m; det är referenspunkten som flyttat.
...
class VesselLifecycleManager {
  ...
  hasCompletedJourney(vessel) {
    ...
    // Helgranskning 2026-07-06 (vlm-pwm-eta#1): även _routeDirection före
    // rå cog — momentan kurs 46–55° i NO-sträckan felklassades som syd och
    // fördröjde elimineringen till timeout-backstoppen.
   ...
   */
  _isNorthbound(cog) {
    // Produktionsredo (2026-07-03): icke-finit COG (null/undefined — AIS
    // "ej tillgänglig") gav inkonsekvent coercion: null <= 45 är true
    // (⇒ norrut) medan undefined ⇒ söderut. Utan kurs är söderut-reglerna
    // det konservativa valet (söderut-fullbordan kräver både Olidebron-
    // passage och lat-gräns, så en okänd-kurs-båt elimineras inte i förtid).
    if (!Number.isFinite(cog)) return false;
    // Northbound: COG between 315° and 45° (through 0°)
    // Fable-granskningen 2026-08-10 (FG-DIR): bandet lyft till den namngivna
    // predikatfamiljen (lib/utils/cogDirection). Gränserna är oförändrade —
    // predikatet läser COG_DIRECTIONS. Finit-gaten ovan står kvar med flit:
    // den bär den binära semantiken (ingen kurs ⇒ behandla som söderut, det
    // konservativa valet), som predikatet inte kan uttrycka.
    return isNorthCog(cog);
  }
  ...
  getJourneyStatus(vessel) {
    ...
    // Helgranskning 2026-07-06: samma låspreferens som hasCompletedJourney.
    const statusLockedDir = vessel._finalTargetDirection || vessel._routeDirection;
    const isNorthbound = statusLockedDir
      ? statusLockedDir === 'north'
      : this._isNorthbound(vessel.cog);
  ...
  _getCompletionReason(vessel, status) {
    ...
    if (status.isLastTarget) {
      // Fable-granskningen 2026-07-10b (SYS-6): etiketten läste rå cog medan
      // BESLUTET (hasCompletedJourney) använder låst riktning — post-passage-
      // drift kunde ge "southbound" i loggen för en norrut-fullbordan
      // (vilseledande rådata för fältprovsauditerna). Samma källa som beslutet.
      const lockedDir = vessel._finalTargetDirection || vessel._routeDirection;
      let direction;
      if (lockedDir) {
        direction = lockedDir === 'north' ? 'northbound' : 'southbound';
      } else {
        direction = this._isNorthbound(vessel.cog) ? 'northbound' : 'southbound';
      }
