...
class BridgeOpeningService {
  ...
  _fire(event, members, due, firedBy, now) {
    ...
    const payload = {
      ...
      // avstånd och ETA kortet visar användaren. Klaffbron#32
      // (2026-08-07T13:46:58.924Z) lovade "om 1 minut" på d=1209 m byggt på ett
      // 854 s gammalt fix, vilket implicerar 39 knop; 2 av 36 fältavfyrade
      // varningar vilade på fix äldre än 10 min. Åldern räknas mot RÅANKARET
      // (se _refreshArm) och vid AVFYRNINGEN, inte vid fixet: deadline-grenen
      // kan fyra långt efter det fix prognosen bygger på. Golvet 0 speglar
      // _refreshArms egen behandling av framtida ankare — en negativ "ålder" är
      // klockskev, inte ålder. RENT ADDITIVT: eta_minutes = -1-grinden över
      // STALE_ETA_HARD är fas B och finns medvetet INTE här.
      fixAgeMs: lead && Number.isFinite(lead.rawAnchorMs)
