...
class SystemCoordinator extends EventEmitter {
  ...
  /**
   * Check if a vessel currently has active coordination (API used by other services)
   * @param {string|number} mmsi - Vessel MMSI
   * @returns {boolean} True if coordination is active for the vessel or system-wide
   */
  hasActiveCoordination(mmsi) {
    try {
      const id = mmsi?.toString();
      if (!id) return false;

      // System-wide coordination overrides
      if (this.globalSystemState?.coordinationActive) return true;

      const state = this.vesselCoordinationState.get(id);
      if (!state) return false;

      // Active flag plus within coordination window
      if (state.coordinationActive) return true;

      // Treat very recent GPS events as active coordination
      const now = Date.now();
      if (state.lastUpdateTime && (now - state.lastUpdateTime) < this.config.gpsEventCooldownMs) {
        return true;
      }
      return false;
    } catch (e) {
  ...
  getCoordination(mmsi) {
    try {
      ...
      let level = 'normal';
      if (state.coordinationType === 'gps_jump') level = 'enhanced';
      else if (state.coordinationType === 'uncertain_position') level = 'moderate';
      else if (state.coordinationType === 'large_movement') level = 'light';
      else if (this.globalSystemState?.coordinationActive) level = 'system_wide';

      const active = !!state.coordinationActive || !!this.globalSystemState?.coordinationActive;
      return {
        level,
  ...
  coordinatePositionUpdate(mmsi, gpsAnalysis, vessel, oldVessel) {
    ...
    // passage-detektion (fryst bridge_text globalt). Decayen baseras nu på en
    // separat lastJumpTime som bara sätts vid faktiska GPS-jumps.
  ...
  _handleGPSJumpEvent(mmsi, coordinationState, recommendation, currentTime) {
    ...
    // F10: spåra tidpunkten för det FAKTISKA hoppet i en separat tidsstämpel.
    // Decayen i _updateGlobalCoordinationState baseras på denna (inte på
    // lastStabilityEvent, som _assessSystemStability skriver om varje tick).
  ...
  _updateGlobalCoordinationState(gpsAnalysis, currentTime) {
    // F10: decay baserad på tiden sedan senaste FAKTISKA GPS-jump (lastJumpTime),
    // INTE lastStabilityEvent (som _assessSystemStability stämplar om varje tick
    // → blockerade decay för evigt → permanent stuck koordination). Om det inte
    // kommit något nytt jump på gpsEventCooldownMs, dekrementera räknaren. Detta
    // gör att en lugn period alltid till slut släpper system-wide-koordinationen.
    const lastJump = this.globalSystemState.lastJumpTime;
    const timeSinceJump = currentTime - (Number.isFinite(lastJump) ? lastJump : currentTime);
    if (timeSinceJump > this.config.gpsEventCooldownMs) {
      this.globalSystemState.unstableGPSCount = Math.max(0, this.globalSystemState.unstableGPSCount - 1);
      // Flytta fram referenspunkten ett kylsteg så varje cooldown-fönster bara
      // dekrementerar en gång (annars skulle ett gammalt lastJump nolla allt på
      // en enda tick).
      this.globalSystemState.lastJumpTime = currentTime;
