...
class SystemCoordinator extends EventEmitter {
  ...
  /**
   * Destroy — släpp alla pending debounce-timers och töm tillståndet.
   * ChatGPT-granskning 2 (CG2-17, 2026-07-11): koordinatorn var enda
   * timer-ägande tjänsten utan destroy() och saknades i onUninit-kedjan —
   * en pending 2 s-debounce kunde överleva nedstängningen (ofarlig callback,
   * men mot kodbasens städmönster).
   */
  destroy() {
    for (const [, debounceData] of this.bridgeTextDebounce.entries()) {
      if (debounceData && debounceData.timer) {
        clearTimeout(debounceData.timer);
      }
    }
    this.bridgeTextDebounce.clear();
