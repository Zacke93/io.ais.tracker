...
class StatusStabilizer {
  ...
  _applyStabilizationLogic(mmsi, proposedStatus, history, vessel, positionAnalysis) {
    const currentTime = Date.now();
    // OBS: proposed status pushas till historiken i stabilizeStatus FÖRE
    // analysen, så recentEntries innehåller alltid minst en post — den gamla
    // "no_history"-grenen var död kod och är borttagen (2026-07-05).
  ...
  _handleGPSJumpStabilization(mmsi, proposedStatus, previousStatus, history, currentTime) {
    ...
    } else if (history.stabilizationActive) {
      // Fönsterfix (2026-07-05): proposed == previous under GPS-hopp betyder
      // att oenigheten fönstret höll emot är LÖST — släpp fönstret här.
      // Tidigare lämnades stabilizationActive=true med den gamla starttiden
      // kvar, så ett NYTT GPS-hopp minuter senare träffade ett redan "utgånget"
      // fönster och fick ingen stabilisering alls. Nu startar nästa hopp med
      // statusändring ett färskt 30 s-fönster.
      history.stabilizationActive = false;
    }
  ...
  cleanup() {
    ...
    // B8-städning (2026-06-09): stabilizationTimers-mappen togs bort — den
    // populerades aldrig någonstans (ingen .set() i hela klassen), så all
    // timer-rensningskod här var död och vilseledande.
