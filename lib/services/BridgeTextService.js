...
class BridgeTextService {
  ...
  /**
   * Phase 3: "Broöppning pågår vid [bro]" — for target/opening bridges
   */
  _buildBridgeOpening(bridgeName, additionalCount) {
    const additional = this._buildAdditional(additionalCount);
    return `Broöppning pågår vid ${bridgeName}${additional}`;
  }

  /**
   * Phase 3 (legacy fallback): "Broöppning pågår vid [bro]" with target info
   * For intermediate bridges under-bridge.
   */
