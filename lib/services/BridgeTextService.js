...
const {
  APPROACH_RADIUS, APPROACHING_RADIUS, BRIDGE_TEXT_CONSTANTS, BRIDGE_SEQUENCE, TARGET_BRIDGES,
} = require('../constants');
...
class BridgeTextService {
  constructor(bridgeRegistry, logger, systemCoordinator = null, vesselDataService = null) {
    ...
    try {
      ...
      const validVessels = vessels.filter((vessel) => {
        ...
        if (!vessel.mmsi || !vessel.name) {
          this.logger.debug(`⚠️ [BRIDGE_TEXT] Skipping vessel with missing critical data: mmsi=${vessel.mmsi}, name=${vessel.name}, status=${vessel.status}, currentBridge=${vessel.currentBridge}, targetBridge=${vessel.targetBridge}`);
          return false;
        }
        return true;
      });
  ...
  _tryRecentlyPassedPhrase(vessel, bridgeName, count, eta) {
    try {
      ...
      this.logger.debug(`🔍 [PASSED_CHECK] ${vessel.mmsi}: status=${vessel.status}, hasTime=${!!vessel.lastPassedBridgeTime}, window=${withinTimeWindow}`);
    ...
    if (
      ...
      if (count === 1) {
        ...
        if (vessel.status === 'waiting') {
          ...
          // CRITICAL FIX: Stallbackabron special case - "En båt närmar sig Stallbackabron på väg mot [målbro]"
        ...
        if (vessel.status === 'waiting') {
          ...
          // CRITICAL FIX: Stallbackabron special case for multiple vessels - "En båt närmar sig Stallbackabron på väg mot [målbro], ytterligare X båtar på väg"
          phrase = `En båt närmar sig Stallbackabron på väg mot ${bridgeName}, ${additionalText} på väg${suffix}`;
        } else if (this._isIntermediateBridge(vessel.currentBridge)) {
          // Only true intermediate bridges (Olidebron, Järnvägsbron) use "vid [bridge] närmar sig" format
    ...
    if (count === 1) {
      ...
      if (priorityVessel.status === 'under-bridge') {
        ...
        if (actualBridge === 'Stallbackabron') {
          // CRITICAL FIX: Only show "på väg mot" if vessel has valid different target bridge
          if (priorityVessel.targetBridge && priorityVessel.targetBridge !== 'Stallbackabron' && this.bridgeRegistry.isValidTargetBridge(priorityVessel.targetBridge)) {
          ...
          // No valid target bridge - vessel leaving canal or has same target as current bridge
          return 'En båt passerar Stallbackabron';
        }
  ...
  _generateMultiVesselPhrase(bridgeName, vessels, priorityVessel, eta) {
      ...
      if (underBridgeCount === 1 && vessels.length > 1) {
        ...
        if (actualBridge === 'Stallbackabron') {
          ...
          if (priorityVessel.targetBridge && priorityVessel.targetBridge !== 'Stallbackabron' && this.bridgeRegistry.isValidTargetBridge(priorityVessel.targetBridge)) {
            const stallbackaETA = this._formatPassedETA(priorityVessel);
            const etaSuffix = stallbackaETA ? `, beräknad broöppning ${stallbackaETA}` : '';
            return `En båt passerar Stallbackabron på väg mot ${priorityVessel.targetBridge}, ${additionalText}${etaSuffix}`;
          }
          // No valid target bridge - vessel leaving canal or has same target as current bridge
          return `En båt passerar Stallbackabron, ${additionalText}`;
        }
    ...
    if (stallbackaWaitingCount > 0) {
      if (stallbackaWaitingCount === vessels.length) {
        // All vessels are "åker strax under" Stallbackabron - use consistent format
        ...
        // Multiple vessels - use "ytterligare X båtar" format for consistency
        ...
        if (priorityVessel.targetBridge && priorityVessel.targetBridge !== 'Stallbackabron' && this.bridgeRegistry.isValidTargetBridge(priorityVessel.targetBridge)) {
          const stallbackaETA = this._formatPassedETA(priorityVessel);
          const etaSuffix = stallbackaETA ? `, beräknad broöppning ${stallbackaETA}` : '';
          return `En båt åker strax under Stallbackabron på väg mot ${priorityVessel.targetBridge}, ${additionalText}${etaSuffix}`;
        }
        return `En båt åker strax under Stallbackabron, ${additionalText}`;
  ...
  _formatPassedETA(vessel) {
    ...
    // CRITICAL FIX: Additional validation to prevent undefined/null issues
    if (etaMinutes === undefined || etaMinutes === null || Number.isNaN(etaMinutes)) {
      etaMinutes = null; // Normalize to null
    }
    ...
    if (vessel.targetBridge && isValidVesselCoordinates(vessel)) {
      // Coordinates already validated by helper function
      ...
      if (targetBridge && Number.isFinite(targetBridge.lat) && Number.isFinite(targetBridge.lon)) {
        try {
          ...
          // ROBUST VALIDATION: Use helper function to validate distance
          ...
          // FIX: Ensure vessel.sog is a valid number before using it
          const sogValue = Number.isFinite(vessel.sog) ? vessel.sog : 4;
          const speed = Math.max(sogValue, 0.5); // Minimum 0.5 knots, default 4 knots

          // FIX: Use helper function for speed validation
          if (!isValidSpeed(speed) || speed <= 0) {
            this.logger.debug(`⚠️ [ETA_FORMAT] ${vessel.mmsi}: Invalid speed value: ${speed}`);
            return null;
          }
          ...
          if (!Number.isFinite(speedMps) || speedMps <= 0) {
            this.logger.debug(`⚠️ [ETA_FORMAT] ${vessel.mmsi}: Invalid speed conversion (${speedMps}m/s)`);
            return null;
          }

          // Use safe division for robust calculation
          const timeSeconds = safeDivision(distance, speedMps);
          if (timeSeconds === null) {
            this.logger.debug(`⚠️ [ETA_FORMAT] ${vessel.mmsi}: Safe division failed for time calculation`);
            return null;
          }

          etaMinutes = safeDivision(timeSeconds, 60); // Convert to minutes
          if (etaMinutes === null) {
            this.logger.debug(`⚠️ [ETA_FORMAT] ${vessel.mmsi}: Safe division failed for minute conversion`);
            return null;
          }
          ...
          // CRITICAL FIX: Always return null on error, never return objects
          return null;
        }
      } else {
  ...
  getNextBridgeAfter(lastPassedBridge, course) {
    ...
    let nextIndex;
    if (isNorthbound) {
      nextIndex = bridgeIndex + 1; // Moving north = next in sequence
    } else {
      nextIndex = bridgeIndex - 1; // Moving south = previous in sequence
    }

    // Check bounds
    if (nextIndex < 0 || nextIndex >= BRIDGE_SEQUENCE.length) {
      return null; // Reached end of canal
    }

    const nextBridgeId = BRIDGE_SEQUENCE[nextIndex];
    const nextBridgeName = this.bridgeRegistry.getBridgeById(nextBridgeId)?.name;

    // Only return target bridges for "precis passerat" messages
    if (TARGET_BRIDGES.includes(nextBridgeName)) {
      this.logger.debug(`🎯 [NEXT_BRIDGE] ${lastPassedBridge} → ${nextBridgeName} (${isNorthbound ? 'north' : 'south'}bound)`);
      return nextBridgeName;
    }

    return null; // Next bridge is not a target bridge
  }
