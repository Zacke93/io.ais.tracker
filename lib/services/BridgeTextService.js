 ...
 *
 * Köer vid en tidigare bro visas utan minutprognos vid den verkliga väntbron,
 * även Olidebron och Järnvägsbron. Båtar under gång får målbrons ETA; bekräftad
 * väntan vid själva målbron ger gruppen ”strax”. Samma båt räknas en gång.
 * Tjänsten har inga egna fas- eller vänttimers.
 *
 * Format per group:
      ...
      for (const target of TARGET_BRIDGES) {
        ...
        if (group && group.length > 0) {
          ...
          for (const vessel of group) {
            ...
            if (bridge === target) moving.push({ ...vessel, _isImminentAtTargetBridge: true });
            else if (!bridge) moving.push(vessel);
