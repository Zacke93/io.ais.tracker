...
class BridgeTextService {
  ...
  _buildGroupPhrase(vessels, targetBridge) {
    ...
    // B6 (körning 2026-07-03, F9): en båt vars target redan ligger i
    // passedBridges får inte driva "strax" — flaggan hör till en passage som
    // redan skett (kvarhängande imminent efter sen/inferrerad detektering).
    const anyImminent = vessels.some((v) => v && v._isImminentAtTargetBridge === true
      && !(v.targetBridge && Array.isArray(v.passedBridges)
