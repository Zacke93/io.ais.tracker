...
const {
  APPROACH_RADIUS, APPROACHING_RADIUS, UNDER_BRIDGE_SET_DISTANCE, UNDER_BRIDGE_CLEAR_DISTANCE,
...
class BridgeTextService {
  constructor(bridgeRegistry, logger, systemCoordinator = null, vesselDataService = null, passageLatchService = null) {
    ...
    this._recentUnderBridgeAnnouncements = new Map();
  ...
  /**
   * Check if vessel has synthetic under-bridge hold active
   * @private
   */
  _hasActiveSyntheticUnderBridge(vessel) {
    return Boolean(
      vessel
  ...
  /**
   * Determine if vessel should be treated as under-bridge for UI purposes.
   * @private
   */
  _isUnderBridgeContext(vessel, bridgeName) {
    if (!vessel) {
      return false;
    }

    if (this._hasValidPendingUnderBridge(vessel, bridgeName)) {
      return true;
    }
    ...
    if (this._hasActiveSyntheticUnderBridge(vessel)) {
      if (!bridgeName || !vessel._syntheticUnderBridgeBridgeName) {
        return true;
      }
      if (vessel._syntheticUnderBridgeBridgeName === bridgeName) {
        return true;
      }
    }
  ...
  /**
   * Determine route direction using available hints (route lock, target, COG).
   * @private
   */
  _determineDirectionHint(vessel) {
    if (!vessel) {
      return null;
    }
    ...
    if (typeof vessel._finalTargetDirection === 'string') {
      return vessel._finalTargetDirection;
    }
  ...
  _generatePhraseForBridge(bridgeName, vessels) {
    ...
    if (this._hasRecentlyPassed(priorityVessel)
        && !this._hasValidPendingUnderBridge(priorityVessel, priorityVessel.lastPassedBridge || bridgeName)) {
  ...
  _logPhraseStats(bridgeName, vessels, priorityVessel, eta) {
    ...
    const underBridge = vessels.filter((v) => this._isUnderBridgeContext(v, v.currentBridge || bridgeName)).length;
  ...
  _tryRecentlyPassedPhrase(vessel, bridgeName, count, eta) {
    try {
      if (this._isUnderBridgeContext(vessel, vessel.currentBridge)) {
        this.logger.debug(`⏸️ [PASSED_SUPPRESSED] ${vessel.mmsi}: Under-bridge context active for ${vessel.currentBridge} - delaying "precis passerat"`);
        return null;
      }
      ...
      if (hasPassedStatus || withinTimeWindow || withinGraceWindow) {
        ...
        if (lastPassedBridge) {
          ...
          if (this._shouldDelayPassedForUnderBridge(vessel, lastPassedBridge)) {
            this.logger.debug(
              `⏸️ [PASSED_SEQUENCE] ${vessel.mmsi}: Waiting for broöppning announcement at ${lastPassedBridge} before "precis passerat"`,
            );
            return null;
          }
          ...
          if (!nextTargetBridge || nextTargetBridge === lastPassedBridge) {
            const directionHint = this._determineDirectionHint(vessel);
            nextTargetBridge = this.getNextBridgeAfter(lastPassedBridge, directionHint ?? vessel.cog ?? 0);
  ...
  _resolveFallbackTargetBridge(vessel, lastPassedBridge) {
    ...
    const directionHint = this._determineDirectionHint(vessel);
    const fallback = this.getNextBridgeAfter(lastPassedBridge, directionHint ?? vessel.cog ?? 0);
    if (fallback) {
      this.logger.debug(
  ...
  _tryIntermediateBridgePhrase(vessel, bridgeName, count, eta) {
    if (
      ...
      if (this._isUnderBridgeContext(vessel, vessel.currentBridge)) {
        ...
        this._recordUnderBridgeAnnouncement(vessel.currentBridge, vessel);
  ...
  _generateStandardPhrase(bridgeName, vessels, priorityVessel, count, eta) {
    ...
    if (count === 1) {
      ...
      if (this._isUnderBridgeContext(priorityVessel, actualBridge) || statusForMessage === 'under-bridge') {
        const prioritizedTarget = priorityVessel.targetBridge || priorityVessel._bridgeTextDerivedTarget;
        const prefersTarget = prioritizedTarget === bridgeName || this._isTargetBridge(bridgeName);
        const displayBridge = prefersTarget ? bridgeName : actualBridge;
        // STALLBACKABRON REFACTORED: Use centralized helper
        if (displayBridge === 'Stallbackabron') {
        ...
        if (this._isTargetBridge(displayBridge)) {
          this._recordUnderBridgeAnnouncement(displayBridge, priorityVessel);
          return `Broöppning pågår vid ${displayBridge}`;
        }
        ...
        this._recordUnderBridgeAnnouncement(displayBridge, priorityVessel);
        return `Broöppning pågår vid ${displayBridge}${etaSuffix}`;
      }
  ...
  _generateMultiVesselPhrase(bridgeName, vessels, priorityVessel, eta) {
    ...
    const underBridgeCount = vessels.filter((v) => this._isUnderBridgeContext(v, v.currentBridge || bridgeName)).length;
    ...
    if (underBridgeCount > 0) {
      ...
      if (underBridgeCount === 1 && vessels.length > 1) {
        ...
        if (this._isTargetBridge(actualBridge)) {
          this._recordUnderBridgeAnnouncement(actualBridge, priorityVessel);
  ...
  _getStallbackabronMessage(vessel, count = 1, additionalCount = 0) {
    ...
    let messageType = this.stallbackabronHelper.getMessageType(vessel, distance || Infinity);
    if (messageType !== 'under-bridge' && this._hasValidPendingUnderBridge(vessel, 'Stallbackabron')) {
      messageType = 'under-bridge';
    }
    ...
    if (messageType === 'under-bridge') {
      this._recordUnderBridgeAnnouncement('Stallbackabron', vessel);
    }
  ...
  _recordUnderBridgeAnnouncement(bridgeName, vessel = null) {
    if (!bridgeName) {
      return;
    }
    this._recentUnderBridgeAnnouncements.set(bridgeName, Date.now());
    if (vessel && vessel._pendingUnderBridgeBridgeName === bridgeName) {
      vessel._pendingUnderBridgeBridgeName = null;
      vessel._pendingUnderBridgeSetAt = null;
    }
  }

  _hasRecentUnderBridgeAnnouncement(bridgeName) {
    if (!bridgeName || !this._recentUnderBridgeAnnouncements.has(bridgeName)) {
      return false;
    }
    const timestamp = this._recentUnderBridgeAnnouncements.get(bridgeName);
    if (!timestamp) {
      return false;
    }
    const ttl = BRIDGE_TEXT_CONSTANTS.UNDER_BRIDGE_SEQUENCE_TTL_MS || 0;
    if (ttl > 0 && (Date.now() - timestamp) > ttl) {
      this._recentUnderBridgeAnnouncements.delete(bridgeName);
      return false;
    }
    return true;
  }

  _hasValidPendingUnderBridge(vessel, bridgeName) {
    if (!vessel || !vessel._pendingUnderBridgeBridgeName) {
      return false;
    }
    if (bridgeName && vessel._pendingUnderBridgeBridgeName !== bridgeName) {
      return false;
    }
    const ttl = BRIDGE_TEXT_CONSTANTS.UNDER_BRIDGE_SEQUENCE_TTL_MS || 0;
    if (ttl > 0 && vessel._pendingUnderBridgeSetAt
        && (Date.now() - vessel._pendingUnderBridgeSetAt) > ttl) {
      vessel._pendingUnderBridgeBridgeName = null;
      vessel._pendingUnderBridgeSetAt = null;
      return false;
    }
  ...
  _shouldDelayPassedForUnderBridge(vessel, bridgeName) {
    if (!bridgeName) {
      return false;
    }
    if (this._hasValidPendingUnderBridge(vessel, bridgeName)) {
    ...
    return !this._hasRecentUnderBridgeAnnouncement(bridgeName);
  }
  ...
  getNextBridgeAfter(lastPassedBridge, courseOrDirection) {
    ...
    if (typeof courseOrDirection === 'string') {
      const normalized = courseOrDirection.toLowerCase();
      if (normalized.startsWith('north')) {
        isNorthbound = true;
      } else if (normalized.startsWith('south')) {
        isNorthbound = false;
      }
    ...
    if (typeof isNorthbound !== 'boolean') {
      return null;
    }
