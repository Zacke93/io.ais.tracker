  ...
  generateBridgeText(vessels) {
    ...
    if (this.systemCoordinator) {
      ...
      if (debounceCheck.shouldDebounce) {
        ...
        return this.lastBridgeText || 'Inga båtar är i närheten av Klaffbron eller Stridsbergsbron';
      }
    }

    if (!vessels || vessels.length === 0) {
      this.logger.debug('❌ [BRIDGE_TEXT] No relevant vessels - returning default message');
      const defaultText = 'Inga båtar är i närheten av Klaffbron eller Stridsbergsbron';
    ...
    if (stallbackaWaitingCount > 0) {
      if (stallbackaWaitingCount === vessels.length) {
        ...
        if (priorityVessel.targetBridge && priorityVessel.targetBridge !== 'Stallbackabron' && this.bridgeRegistry.isValidTargetBridge(priorityVessel.targetBridge)) {
          const stallbackaETA = this._formatPassedETA(priorityVessel);
          const etaSuffix = stallbackaETA ? `, beräknad broöppning ${stallbackaETA}` : '';
          return `${countText} åker strax under Stallbackabron på väg mot ${priorityVessel.targetBridge}${etaSuffix}`;
        }
        // No valid target bridge - vessel leaving canal or has same target as current bridge
        return `${countText} åker strax under Stallbackabron`;
      }
