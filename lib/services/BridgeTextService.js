...
class BridgeTextService {
  ...
  generateBridgeText(vessels) {
    try {
      ...
      // hela stacken, så fällan går att se i en fältlogg i stället för att
      // yttra sig som en oförklarlig "Inga båtar".
  ...
  _logSwallowedError(error, vessels) {
    ...
    try {
      this.logger.error(
        ...
        + `trots att båtar fanns i indatan. fartyg=[${vilka}] orsak=${orsak}${stack}`,
      );
    } catch (loggfel) {
