...
class BridgeTextService {
  ...
  _generateStandardPhrase(bridgeName, vessels, priorityVessel, count, eta) {
    ...
    if (count === 1) {
      ...
      if (statusForMessage === 'under-bridge') {
        const prefersTarget = priorityVessel.targetBridge === bridgeName
      ...
      if (statusForMessage === 'en-route' && priorityVessel.targetBridge === bridgeName) {
  ...
  _shouldShowWaiting(vessel, bridgeName, statusOverride = undefined) {
    ...
    this.logger.debug(`🔍 [WAITING_CHECK] ${vessel.mmsi}: NOT waiting at ${bridgeName} - targetBridge: ${vessel.targetBridge}, currentBridge: ${vessel.currentBridge}, status: ${status}`);
    return false;
  }
