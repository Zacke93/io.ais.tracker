...
class BridgeTextService {
  constructor(bridgeRegistry, logger, proximityService = null) {
    this.bridgeRegistry = bridgeRegistry;
    this.logger = logger;
    this.proximityService = proximityService; // Optional for better distance calculations
  ...
  _getDistanceToTargetBridge(vessel) {
    try {
      ...
      if (!vessel || !vessel.lat || !vessel.lon) {
        this.logger.debug(
          `⚠️ [DISTANCE_CALC] ${vessel?.mmsi || 'unknown'}: Missing vessel position data`,
        );
        return null;
      }
  ...
  _generateStandardPhrase(bridgeName, vessels, priorityVessel, count, eta) {
    if (count === 1) {
      ...
      // 1. "Broöppning pågår" (≤50m) - ONLY WITH REAL UNDER-BRIDGE STATUS
      // CRITICAL: Never show "pågår" unless we actually observed vessel <= 50m from bridge
      if (priorityVessel.status === 'under-bridge') {
        ...
        // AUTHENTIC: Only show "pågår" for real under-bridge observations
  ...
  _shouldShowWaiting(vessel, bridgeName) {
    ...
    if (vessel.currentBridge === bridgeName && vessel.distanceToCurrent !== undefined && vessel.distanceToCurrent <= APPROACH_RADIUS) {
