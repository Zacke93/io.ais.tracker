...
const { APPROACH_RADIUS, APPROACHING_RADIUS, BRIDGE_TEXT_CONSTANTS } = require('../constants');
...
class BridgeTextService {
  ...
  generateBridgeText(vessels) {
    try {
      ...
      // ENHANCED: Check for debouncing with SystemCoordinator
      if (this.systemCoordinator) {
        ...
        if (debounceCheck.shouldDebounce) {
          this.logger.debug(
            `⏸️ [BRIDGE_TEXT_DEBOUNCED] Debouncing bridge text update - ${debounceCheck.reason} `
            + `(remaining: ${(debounceCheck.remainingTime / 1000).toFixed(1)}s)`,
          );
