...
// Status hysteresis constants for reducing UI pendling and zone transition stability
const STATUS_HYSTERESIS = {
  // Approaching zone (500m nominal with hysteresis)
  ...
  // Waiting zone (300m nominal with hysteresis)
  WAITING_SET_DISTANCE: 280, // meters - distance to activate "inväntar broöppning" status
  WAITING_CLEAR_DISTANCE: 320, // meters - distance to clear "inväntar broöppning" status (prevents pendling)

  // Under-bridge zone (50m/70m already defined above)
  // UNDER_BRIDGE_SET_DISTANCE: 50m, UNDER_BRIDGE_CLEAR_DISTANCE: 70m

  // Zone transition capture timings
  CRITICAL_TRANSITION_HOLD_MS: 3000, // 3 seconds - hold critical transitions (åker strax under, under-bridge)
  ZONE_TRANSITION_GRACE_MS: 1500, // 1.5 seconds - grace period for zone boundary crossings
};
