...
class AISSourceMultiplexer extends EventEmitter {
  ...
  constructor(logger, settingsStore = null) {
    ...
    // En ny omställning eller nedstängning ogiltigförklarar äldre awaits.
    // _disposed ensam skyddar inte när muxen redan hunnit startas igen.
   ...
   *
   * KONTRAKT för perFeed.aishub:s två tidsstämplar (N30, runda 5):
   *  - lastPollStartedAt = när en poll SENAST STARTADE (null före den första).
   *  - pollChainArmedAt  = när kedjan BEORDRADES starta, dvs. klientens
   *    connect (null när klienten saknas eller är stoppad).
   * Bägge är finita tal eller null — aldrig 0, eftersom läsaren mäter
   * `now - stämpeln`. Feed-vaktens kedjedödsgren ska läsa lastPollStartedAt
   * och FALLA TILLBAKA på pollChainArmedAt när den förra saknas; annars är
   * vakten inert under hela fönstret mellan connect och första pollen.
   * @returns {object}
   */
  getConnectionStats() {
    ...
    return {
      ...
      perFeed: {
        ...
        aishub: {
          ...
          // N30 (helkodsgranskning runda 5): ARMERINGSÖGONBLICKET — när
          // pollkedjan beordrades starta (klientens connect), till skillnad
          // från lastPollStartedAt som är när en poll faktiskt startade.
          // Finit tal medan kedjan är armerad, annars null (aldrig 0).
          // VARFÖR fältet finns: mellan connect() och första pollen är
          // lastPollStartedAt null, och feed-vaktens kedjedödsgren kräver ett
          // finit värde — en kedja som dör FÖRE sin första poll var därför
          // osynlig för vakten i evighet. app.js:_checkAishubFeedHealth ska
          // falla tillbaka på det HÄR fältet när lastPollStartedAt saknas.
          // Stämpeln är flyktig och sätts i connect: den persisterade
          // configuredSince duger INTE (den ser gammal ut direkt vid boot och
          // hade gett en falsk kick).
          pollChainArmedAt: hub && Number.isFinite(hub.pollChainArmedAt) ? hub.pollChainArmedAt : null,
  ...
  async _reconcile(prev = {}) {
    ...
    // Kontrollera FÖRE även teardown: en äldre omställning får varken skapa
    // en bortvald källa eller riva den nya källa användaren just valt.
    if (this._disposed || generation !== this._reconcileGeneration) return;
    ...
    // Jämför mot det verkliga barnet. En mellanliggande konfiguration kan
    // ha ersatts under stream-await innan dess username nådde hubben.
    const hubCredChanged = this._hubClient
      && this._hubClient.username !== this._config.aishubUsername;
    ...
    // disconnect/nyare reconcile äger redan eventuell städning. Den gamla
    // fortsättningen får inte riva ett barn som hör till en ny generation.
    if (this._disposed || generation !== this._reconcileGeneration) return;
  ...
  _fuseAndEmit(feed, msg) {
    ...
    if (!verdict.accept) {
      ...
      this._captureRejectedSourceSample(feed, msg, all.reasons, now);
  ...
  /**
   * Fältprov: bevara redan mottagna stream-positioner som fusionen avvisar.
   * AISHubs motsvarighet finns i AISHUB_RESPONSE_SAMPLE före urvalet.
   * Detta är separat diagnostik, aldrig AIS_REPLAY_SAMPLE eller ett nytt
   * livstecken. En avvisad fix måste granskas med sin avslagsorsak innan den
   * kan stödja något passagebevis. Inga nya anrop, timers eller lagrat state.
   * Endast full-debug: en rad per avslag bevarar luckans första/sista fix.
   * @private
   */
  _captureRejectedSourceSample(feed, msg, reasons, now) {
    if (feed !== 'aisstream' || this.logger.debugLevel !== 'full') return;
    this.logger.log('[AIS_SOURCE_REJECT_SAMPLE]', JSON.stringify({
      mmsi: String(msg.mmsi),
      feed,
      lat: Number.isFinite(msg.lat) ? msg.lat : null,
      lon: Number.isFinite(msg.lon) ? msg.lon : null,
      sog: Number.isFinite(msg.sog) ? msg.sog : null,
      cog: Number.isFinite(msg.cog) ? msg.cog : null,
      fixTs: Number.isFinite(msg.fixTs) ? msg.fixTs : null,
      // AISstream saknar äkta fixtid: båda källstämplarna avser mottagning.
      fixTsQuality: 'receipt',
      receivedAt: Number.isFinite(msg.timestamp) ? msg.timestamp : null,
      capturedAt: now,
      reasons,
    }));
  }
  ...
  _logFusionReject(mmsi, reasons, meta) {
    ...
    this.logger.debug(
      ...
      + '(urval: högst 1/MMSI/5min; alla avslag räknas i FUSION_HEALTH)',
    );
  }
  ...
  /**
   * Rutnyckel för positionsindexet (1e-5° ≈ 1,1 m i lat).
   * @private
   */
   ...
   * sida om en rutgräns. Även ett fast 3×3-svep är för snävt: 1,5 m kan
   * korsa två latitudrutor och tre longitudrutor här. Pröva därför fartygets
   * samtliga indexposter. Indexet har redan TTL och storlekstak; endast
   * samma MMSI kräver avståndsräkning.
   * @private
  ...
  _findShadowCounterpart(feed, msg) {
    ...
    for (const [key, entry] of this._shadowPosIndex) {
      ...
      // Färskaste motparten vinner, oavsett indexets insättningsordning.
      if (!best || side.storedAt > best.side.storedAt) {
        best = {
   ...
   *      rapporter.
   *   2. anroparen förbrukar BÅDA leveranserna i ett godkänt par, så en
   *      och samma post aldrig kan paras om mot nyare motparter.
   * @private
   * @returns {boolean} true när tidsgrinden godkände paret
   */
  _recordShadowPair(streamSide, hubSide) {
    const w = this._shadowWindow;
    const cfg = AIS_CONFIG.SHADOW;
    const receiptTs = streamSide.deliveryTs;
    if (!Number.isFinite(receiptTs) || !Number.isFinite(hubSide.fixTs)) return false;
    ...
    if (w.fixLags.length >= cfg.MAX_SAMPLES_PER_WINDOW) return true;
  ...
  _recordShadowSample(feed, msg) {
    ...
    if (Number.isFinite(msg.lat) && Number.isFinite(msg.lon)) {
      ...
      let paired = false;
      if (counterpart) {
        paired = feed === 'aisstream'
          ? this._recordShadowPair(side, counterpart.side)
          : this._recordShadowPair(counterpart.side, side);
      ...
      // Båda leveranserna förbrukas av ett godkänt par. Det inkommande
      // samplet lades tidigare tillbaka och kunde räknas en gång till,
      // även i nästa rapportfönster. Ett avvisat par behåller däremot den
      // nya leveransen så att en senare, färsk motpart fortfarande kan hittas.
      if (!paired) {
  ...
  _emitShadowCompare(isFinal = false) {
    ...
    // K32(k) (fältprov 10, 2026-08-19): "window=5min" stod ensamt i radens
    // huvud och lästes som att ALLA mått på raden var begränsade till fem
    // minuter — varpå ett maxSilence på 32 min på en 5-minutersrad såg ut som
    // ett trasigt instrument. Fem minuter gäller räknarna och NÄR paren
    // bokförs; motpartens sampel kan finnas kvar från föregående fönster.
    // maxSilence* har en egen tidsbas (se legenden vid fältet).
    // Endast sträng — fältnamn, tal och ordning är oförändrade.
    this.logger.log(
      `🔭 [SHADOW_COMPARE]${isFinal ? ' (slutspolning)' : ''} window=5min `
      + '(räknare i fönstret; par bokförda i fönstret kan ha äldre motpart; maxSilence* har egen tidsbas) '
      ...
      // K32(k): TIDSBASEN utskriven. Glappet mäts per MMSI mot _shadowLastSeen,
      // som ÖVERLEVER fönsterbyten (prunas först vid LAST_SEEN_TTL_MS = 4 h, se
      // _recordShadowSample + pruneFusionState). Fönstret avgör bara VILKA
      // glapp som redovisas här — de som SLÖT SIG i det — aldrig hur långa de
      // får vara: fältprov 2 mätte 669 007 ms i ett enda 5-minutersfönster.
      + `maxSilenceAisstreamMs=${silenceField('aisstream')} maxSilenceAishubMs=${silenceField('aishub')} `
      + '(största glapp per MMSI mellan två meddelanden från SAMMA källa, bokfört när det '
      + 'senare meddelandet kom — glappet kan spänna flera fönster och är INTE kapat till 5 min) '
  ...
  _emitFusionHealth() {
    ...
    const skewLegend = (lagMin !== null && lagMin < 0)
      ...
      : '';
    // K32(k), andra halvan (fältprov 10, 2026-08-19): klockblocket hade EN
    // etikett ("| klocka:") men bär TVÅ tidsskalor — och raden i övrigt är
    // uttryckligen uppdelad i "fönster:" och "totalt:", så läsaren antog att
    // hela blocket var fönstermått.
    //  • hubLagMin/Median/Offset kommer ur clock.hubLags (och pairLags), som
    //    pushClockSample håller i ett GLIDANDE fönster på
    //    FUSION.CLOCK_OFFSET_WINDOW_MS (30 min) — de faller alltså tillbaka
    //    när klockan rättas, vilket är hela poängen med tidsfönstret.
    //  • hubAheadSamples och hubPairsDroppedStale är rena ++-räknare på clock
    //    (FixFusionPolicy.observeClock) som ALDRIG nollställs — ett
    //    skevutbrott från i förrgår står kvar i dem resten av körningen.
    // Skalorna namnges nu var för sig. Endast sträng: fältnamn och tal är
    // oförändrade, och fönsterlängden HÄRLEDS ur konstanten i stället för att
    // skrivas som en siffra som kan glida isär från koden.
    this.logger.log(
      ...
      + `| klocka/${Math.round(AIS_CONFIG.FUSION.CLOCK_OFFSET_WINDOW_MS / 60000)}min glidande fönster: `
      + `hubLagMinMs=${lagMin ?? '-'} hubLagMedianMs=${lagMed ?? '-'} `
      + `hubOffsetMs=${clk.hubOffsetMs} `
      + `| klocka/livstid (nollställs aldrig): hubAheadSamples=${clk.hubAheadSamples} `
