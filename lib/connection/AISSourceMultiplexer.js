...
class AISSourceMultiplexer extends EventEmitter {
  ...
  getConnectionStats() {
    ...
    const lastMsgCandidates = nums([stream?.lastMessageTime, hub?.lastMessageTime]);

    return {
      isConnected: this._computeConnected(),
      reconnectAttempts: maxOf([stream?.reconnectAttempts, hub?.reconnectAttempts]),
      lastMessageTime: lastMsgCandidates.length ? Math.max(...lastMsgCandidates) : null,
      uptime: maxOf([stream?.uptime, hub?.uptime]),
      timeSinceLastMessage: minOfNullable([stream?.timeSinceLastMessage, hub?.timeSinceLastMessage]),
      perFeed: {
        aisstream: {
  ...
  _computeConnected() {
    ...
    const hubOk = !!this._hubClient && this._hubClient.isConnected;
