...
class AISSourceMultiplexer extends EventEmitter {
  ...
  _emitShadowCompare(isFinal = false) {
    ...
    // ett trasigt instrument. Fem minuter gäller räknarna och paren; maxSilence*
    // har en egen tidsbas (se legenden vid fältet). Skalan står nu utskriven.
    // Endast sträng — fältnamn, tal och ordning är oförändrade.
    this.logger.log(
      `🔭 [SHADOW_COMPARE]${isFinal ? ' (slutspolning)' : ''} window=5min `
      + '(fönstret gäller räknarna och paren nedan; maxSilence* mäts på annan tidsbas) '
