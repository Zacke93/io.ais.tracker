 ...
 * fartygen, bakom ett minimiurval. Returnerar null när underlaget inte
 * räcker; då driver bevis A ingen kompensation alls — samma regel som
 * CLOCK_PAIR_MIN_SAMPLES ger bevis B ("kompensera aldrig på gissningar").
 * @private
 * @param {Array<{v: number, mmsi: string|null}>} hubLags
 * @param {object} cfg - AIS_CONFIG.FUSION
 * @returns {number|null} skattat leveranslagg-golv, eller null
 */
function estimateHubLagBound(hubLags, cfg) {
  ...
  if (perVessel.size < minVessels) return null;
...
function observeClock(clock, state, msg, feed, now, cfg) {
  ...
  const lagBound = estimateHubLagBound(clock.hubLags, cfg); // bevis A
  if (lagBound !== null) bound = Math.min(bound, lagBound);
...
function classifyAll(state, msg, now, cfg, ctx) {
  ...
  if (feed === 'aishub') {
    if (clockSkew) reasons.push('hub_clock_skew');
