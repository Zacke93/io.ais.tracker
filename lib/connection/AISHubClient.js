...
class AISHubClient extends EventEmitter {
  ...
  constructor(logger, settingsStore = null) {
    ...
    this._counters = {
      ...
      futureJunk: 0,
      // Gamla cacheposter får aldrig förnya appens positionsklocka.
      staleFixes: 0,
  ...
  _handleGoodEnvelope(parsed, httpMs) {
    ...
    for (const rec of parsed.records) {
      ...
      // Samma 12-minutersgräns som fusionens F4b, också i solo-AISHub.
      // Efter dedup-TTL kunde en oförändrad gammal TIME annars emitteras
      // igen och förnya vessel.timestamp trots avstängd AIS. Grinden måste
      // ligga före dedup-skrivningen. Fusionens klockoffset är alltid <= 0:
      // den kan göra ett rått fix äldre, aldrig rädda en post som faller här.
      // Källsvarets hälsa bokfördes ovan och påverkas inte av gamla positioner.
      if (now - rec.fixTs > this._cfg.MAX_FIX_AGE_MS) {
        this._counters.staleFixes++;
        sweepStaleFixes++;
        continue;
      }
    ...
    this.logger.log(
      ...
      + `futureJunk=${sweepFutureJunk} staleFixes=${sweepStaleFixes} httpMs=${httpMs} `
      + `medianFixAgeMs=${median ?? '-'} p90FixAgeMs=${p90 ?? '-'} `
      + `dupesTotal=${this._counters.dupes} outOfBoxTotal=${this._counters.outOfBox} `
      + `futureJunkTotal=${this._counters.futureJunk} staleFixesTotal=${this._counters.staleFixes}`,
    );
    ...
    fresh.forEach((rec, i) => {
      const timer = setTimeout(() => {
        ...
        if (this._stopped) return;
        // Batchspridning eller eventloopstall kan åldra ett godkänt fix
        // över samma gräns innan det når appens positionsklocka.
        const fixAgeMs = Date.now() - rec.fixTs;
        if (fixAgeMs > this._cfg.MAX_FIX_AGE_MS) {
          this._counters.staleFixes++;
          this.logger.log(`⚠️ [AISHUB_STALE_EMIT] ${rec.mmsi}: fix ${Math.round(fixAgeMs / 1000)} s gammalt vid emission — släpps inte vidare`);
          return;
        }
