  ...
  _handleGoodEnvelope(parsed, httpMs) {
    ...
    for (const rec of parsed.records) {
      ...
      if (Number.isFinite(lastFix) && rec.fixTs <= lastFix) {
        ...
        sweepDupes++;
        // ABSOLUTBELOPP, inte `now − fixTs`: parsern klampar inte framtida
        // TIME-fält, och en post med skräpklocka långt fram i tiden hade med
        // ett ensidigt test gett negativ "ålder" ⇒ evigt livstecken för ett
        // fartyg som kanske aldrig sänder igen. Skevheten är symmetrisk och
        // marginalen (FIX_AGE_CLOCK_SKEW_MARGIN_MS) ingår redan i gränsen.
        if (Number.isFinite(rec.fixTs)
            && Math.abs(now - rec.fixTs) < this._cfg.SEEN_MAX_FIX_AGE_MS) {
          this._counters.seen++;
