...
class AISStreamClient extends EventEmitter {
   ...
   * ("120") och en HTTP-datumsträng. Båda stöds; allt annat ger null så att
  ...
  _parseRetryAfter(raw) {
    ...
    // Ett datum i det förflutna betyder "nu" — men klampningen nedan lyfter
    // ändå upp värdet till golvet, så 0 är säkert här.
  ...
  _extractAISData(message) {
    ...
    const sog = (Number.isFinite(rawSog) && rawSog >= 102.15) ? null : rawSog;
  ...
  _rateLimitDelay() {
    ...
    if (retryAfterMs !== null) {
      const clamped = Math.min(
        Math.max(retryAfterMs, AIS_CONFIG.RATE_LIMIT_RETRY_AFTER_MIN_MS),
  ...
  _scheduleReconnect() {
    ...
    if (rateLimit) {
      ...
      this.logger.log(
        ...
        + `cooldown ${(delay / 60000).toFixed(1)} min (nr ${this._rateLimitCount} i episoden; `
        + `steget står kvar på ${this.reconnectAttempts})`,
      );
