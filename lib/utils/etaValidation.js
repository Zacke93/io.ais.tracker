...
// Helgranskning 2026-07-10: formatETA (bucketing/timmar-format) raderad —
// noll anrop i produktion och tester; all ETA-textning går via SSOT-hjälparen
// formatETABroOpeningClause nedan.
...
function formatETABroOpeningClause(etaMinutes, options = {}) {
  ...
  //
  // C11/U1-KONTRAKTET (etapp 7, 2026-08-09): grenen övertrumfar ÄVEN ogiltig
  // ETA — den får därför bara nås med FÄRSK position (≤ STALE_ETA_HARD).
  // Färskhetsgrinden ligger MEDVETET hos anroparen och inte här: `imminent` är
  // en grupp-OR över flera fartyg i BridgeTextService (`anyImminent`), och en
  // gruppflagga har ingen egen positionsålder att grinda på. Anroparledet:
  //   1. app.js `_reevaluateVesselStatuses` sätter aldrig flaggan på position
  //      äldre än STALE_ETA_HARD (`dataIsFreshEnough`, klocka
  //      `_lastConfirmedPositionMs`);
  //   2. BridgeTextService `_buildGroupPhrase` kräver dessutom
  //      `hasFreshPosition(v)` vid KONSUMTIONEN (C11-spärren i djupled).
  // Ny anropare som skickar `imminent: true` MÅSTE bära samma grind.
