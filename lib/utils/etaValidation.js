...
/**
 * Review fix H2: SSOT for the "beräknad broöppning ..." clause used in
 * bridge_text. Previously the 30-min clamp only lived in
 * BridgeTextService._formatETAAsBroOpening, leaving fallback paths and Flow
 * tokens exposing unclamped values like "om 106 minuter".
 * Any path that converts an etaMinutes number into a Swedish bro-opening
 * clause must go through this helper.
 *
 * @param {number|null|undefined} etaMinutes
 * @returns {string} Swedish clause without trailing punctuation
 */
const ETA_CLAMP_THRESHOLD_MINUTES = 30;
...
function formatETABroOpeningClause(etaMinutes) {
  ...
  if (rounded > ETA_CLAMP_THRESHOLD_MINUTES) {
...
/**
 * Review fix H2: clamp a numeric ETA value to a user-facing maximum so
 * Flow token `eta_minutes` does not carry absurd values (60, 82, 106) for
 * near-stationary vessels. Returns null for invalid input so downstream code
 * can choose whether to expose the token.
 */
function clampETAMinutesForDisplay(etaMinutes) {
  if (!isValidETA(etaMinutes)) return null;
  if (etaMinutes > ETA_CLAMP_THRESHOLD_MINUTES) return ETA_CLAMP_THRESHOLD_MINUTES;
