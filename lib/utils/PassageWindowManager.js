...
class PassageWindowManager {
   ...
   * @returns {number} Combined display window in milliseconds (60s)
   */
  getDisplayWindow() {
  ...
  getInternalGracePeriod(vessel) {
    ...
    if (speed > 5) {
      this.logger.debug(
        `⏱️ [PASSAGE_WINDOW] Fast vessel ${vessel?.mmsi} (${speed.toFixed(1)}kn) - using 3 min internal grace period`,
      );
      return PASSAGE_TIMING.FAST_VESSEL_PASSED_WINDOW;
    }
    this.logger.debug(
      `⏱️ [PASSAGE_WINDOW] Slow vessel ${vessel?.mmsi} (${speed.toFixed(1)}kn) - using 3 min internal grace period`,
    );
    return PASSAGE_TIMING.FAST_VESSEL_PASSED_WINDOW; // System stability > display window
