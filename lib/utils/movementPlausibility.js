...
function allowedMovement(vessel, oldVessel, floorM) {
  ...
  if (!Number.isFinite(dtMs) || dtMs <= 0) {
    return {
      allowedM: floorM, dtMs, dtSource: source, speedFloorKn: null,
    };
  }
  ...
  return {
    allowedM: Math.max(floorM, physicalM), dtMs, dtSource: source, speedFloorKn,
  };
}
 ...
 * @param {Object} vessel - nyare sampel
 * @param {Object} oldVessel - äldre sampel
 * @param {number} floorM - anropsställets egen tröskel
