...
// Fable-granskningen 2026-08-10 (FG-E3): isWithinRadius(vessel, bridge, radius)
// raderad — noll referenser i hela repot (prod och test), enda träffarna var
// definitionen och exportraden. VARFÖR den aldrig användes: varje avståndsgrind
// i appen behöver SJÄLVA avståndet (för texter, sortering och hysteres), så
// anroparna kallar calculateDistance direkt och jämför mot sin egen tröskel.
// Den finita-kontrollen som stod här (null <= radius är sant i JS) lever kvar
// hos anroparna via geometry-null-safety-sviten.
...
function detectBridgePassage(vessel, oldVessel, bridge, opts = {}) {
  ...
  // V2 (A/B-nattkörningen 2026-08-03, omarbetad i granskningsrunda 2): sidbytet
  // mäts mellan TVÅ sampel. Vid tät sampling (dubbelkälla ~68 s) kan fixen som
  // landar inne i under-bro-zonen redan ligga BORTOM brolinjen — korsningen låg
  // då i INTRÄDESSTEGET och utgångssteget (t.ex. +16 m → +123 m) visar ingen
  // sidväxling alls, så passagen tappades helt (TIM 2026-08-02: Olidebron OCH
  // Järnvägsbron). StatusService korrigerar då zonbesökets INTRÄDESANKARE till
  // den sida båten kom ifrån och stämplar bron; här jämförs den nuvarande
  // positionen mot det ankaret — alltså NETTOSIDBYTET över hela zonbesöket, i
  // stället för mellan två godtyckliga sampel.
  //
  // NETTOKRAVET ÄR AVGÖRANDE, inte kosmetiskt: en ren stämpel ("bron korsades
  // någon gång") hade fabricerat en passage för varje båt som korsar och
  // VÄNDER TILLBAKA (U-sväng, avbruten insegling) eller som brusar över linjen
  // och driftar ut på sin egen sida — exakt den klass hasChangedBridgeSide en
  // gång infördes för (CLABBYDOO). Med nettokravet ger båda fallen samma sida
  // och METHOD 1 fyrar inte.
  //
  // Gäller ENDAST METHOD 1 (vars egna villkor kräver att föregående fix låg
  // inne i zonen ≤50 m och att båten nu rör sig bort >60 m). Metoderna 4/5/6
  // behåller det råa sidbyteskravet. Sedan breddningen 2026-08-03
  // (UNDER_BRIDGE_CROSS_PROOF_REQUIRED_FEED = null, användarbeslut) sätts
  // stämpeln oavsett källa — de fem historiska enkelkällemissarna (NORDIC
  // SOLA, ORANESS, CATHARINA, ALICE, EXCALIBUR) är återvunna och golden-
  // texten i fyra korpusar omlåst; se härledningen i constants.js.
  const zoneCrossProven = Boolean(
    vessel._underBridgeCrossedBridge && bridge.name
    && vessel._underBridgeCrossedBridge === bridge.name
