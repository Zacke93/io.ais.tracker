...
function hasCrossedBridgeLine(prevPos, currPos, bridge, opts = {}) {
  ...
  // Project vessel positions onto the perpendicular axis (along canal)
  // This gives us the "along-canal" coordinate for each position
  const projPrev = (prevPos.lat - bridge.lat) * Math.cos(perpBearing)
                 + (prevPos.lon - bridge.lon) * Math.sin(perpBearing);
  const projCurr = (currPos.lat - bridge.lat) * Math.cos(perpBearing)
                 + (currPos.lon - bridge.lon) * Math.sin(perpBearing);
