 ...
 * @param {number} opts.axisBearing - Bridge axis bearing in degrees (future use)
...
function hasCrossedBridgeLine(prevPos, currPos, bridge, opts = {}) {
  ...
  // For North-South oriented canal, use simplified latitude crossing
  // Bridge line is approximated as a horizontal line at bridge.lat
  const prevLat = prevPos.lat;
  const currLat = currPos.lat;
  const bridgeLat = bridge.lat;
  
  // Check if the vessel crossed the bridge latitude line
  const crossedNorthbound = prevLat < bridgeLat && currLat > bridgeLat;
  const crossedSouthbound = prevLat > bridgeLat && currLat < bridgeLat;
  
  return crossedNorthbound || crossedSouthbound;
}
