...
class GPSJumpAnalyzer {
  ...
  /**
   * Etapp 0 AISHub-förberedelse (2026-08-02): fixtids-dt när BÅDA samplen
   * bär fixtid från SAMMA källa. Klockdomänsinvarianten: mottagningstid och
   * äkta fixtid får ALDRIG blandas i en tidsjämförelse — en pollande källa
   * levererar fix som är upp till minuter äldre än mottagningen, och ett
   * blandat dt dömer legitim rörelse som fysiskt omöjlig (eller tvärtom).
   * Returnerar null när fixtids-dt inte är beräkningsbart → anroparen
   * faller tillbaka på dagens mottagningstidsuttryck.
   * @private
   */
  static _fixDtMs(currentVessel, oldVessel) {
    if (!currentVessel || !oldVessel) return null;
    if (!currentVessel.fixFeed || currentVessel.fixFeed !== oldVessel.fixFeed) return null;
    if (!Number.isFinite(currentVessel.fixTs) || !Number.isFinite(oldVessel.fixTs)) return null;
    return currentVessel.fixTs - oldVessel.fixTs;
  }
  ...
  analyzeMovement(mmsi, currentPosition, previousPosition, currentVessel, oldVessel) {
    ...
    // Etapp 0: fixtids-dt inom samma källa; annars mottagningstid som förut.
    const medFixDtMs = GPSJumpAnalyzer._fixDtMs(currentVessel, oldVessel);
  ...
  _analyzeLargeMovement(mmsi, currentPosition, previousPosition, currentVessel, oldVessel, result) {
    // Etapp 0: fixtids-dt inom samma källa; annars mottagningstid som förut.
    const fixDtMs = GPSJumpAnalyzer._fixDtMs(currentVessel, oldVessel);
  ...
  _checkSpeedConsistency(movementDistance, currentVessel, oldVessel) {
    ...
    // pollande källas legitima rörelse som inkonsistent.
    const fixDtMs = GPSJumpAnalyzer._fixDtMs(currentVessel, oldVessel);
