...
function parseTimeToMs(raw) {
  ...
  if (m) {
    ...
    // fält (månad 13 → januari året efter, "99:99:99" → dygn framåt) och gav
    // ett FINIT men fel epoch. Kontraktet lutade sig på F4a/F4b-grindarna i
    // fusionspolicyn — men solo-AISHub går FÖRBI fusionen (muxens
    // pass-through), och ett framtidsrullat fixTs kunde där förgifta
    // dedup-kartan (se framtidsgrinden i AISHubClient._handleGoodEnvelope).
    // Rundturskontrollen gör kalenderskräp till timeParseFail: TIME-larmet
    // (3 svep i rad) ser formatdriften i stället för att posten smiter igenom.
    const rt = new Date(ms);
    if (rt.getUTCFullYear() !== y || rt.getUTCMonth() !== mo - 1
        || rt.getUTCDate() !== d || rt.getUTCHours() !== h
        || rt.getUTCMinutes() !== mi || rt.getUTCSeconds() !== s) {
      return null;
    }
    return ms;
  }
...
function normalizeRecord(raw, stats) {
  ...
  // J6 + J35 (helkodsgranskning runda 2, 2026-08-22): SOG-, COG- och
  // NAVSTAT-reglerna nedan är numera EN implementation i
  // lib/utils/aisFieldNormalization.js som BÅDA ingångarna (den här parsern
  // och AISStreamClient) anropar. Reglerna var skrivna två gånger och hade
  // glidit isär — strömsidan saknade både COG-intervallgrinden och
  // NAVSTAT 15-saneringen. Semantiken här är oförändrad in i minsta
  // gränsfall (tests/aishub-parser-unit.test.js låser den); det som flyttat
  // är VAR den bor. KOERCIONEN stannar kvar hos anroparen: num() är AISHubs
  // wire-kontrakt ('5' → 5), inte en generell AIS-regel.
  //
  // SOG-sentinel: 102.4 = "ej tillgänglig" (format=1-skalan). Samma gräns
  // >= 102.15 som AISStreamClient (täcker även 102.2 "eller mer").
  //
  // FINITGRINDEN STÅR HÄR, INTE I HJÄLPAREN (granskningen av runda 2): den är
  // AISHubs WIRE-KONTRAKT, inte en AIS-regel. num() ger NaN för både saknat
  // fält och skräpsträng — och eftersom Number(null) är 0 hade ett saknat
  // SOG annars blivit "verklig nollfart" (osynliga-båtar-klassen; låst i
  // tests/aishub-parser-unit.test.js). Strömsidan har typad JSON och ska
  // enligt det DEMENTERADE fyndet J3 fortsätta skicka ett icke-finit värde
  // vidare orört, så en grind i hjälparen hade tyst ändrat DESS semantik.
  const sogNum = num(raw.SOG);
  const sog = Number.isFinite(sogNum) ? normalizeSog(sogNum) : null;

  // COG-sentinel + INTERVALLGRIND. Giltig COG är 0-359,9 grader (ITU-R
  // M.1371); 360 är sentinelen "kurs ej tillgänglig" (aldrig fabricerad
  // nordkurs).
  //
  // H34 (helkodsgranskning runda 1, 2026-08-22): grinden fångade BARA exakt
  // 360. Rå COG 3601-4095 avkodas till 360,1-409,5 och slank därför igenom
  // som en "kurs". Nedströms är det appens _validateAISMessage som fångar
  // värdet — men muxen bokför fixen via applyAccept FÖRE appvalideringen, så
  // fusionens F1/F2/F6-state är det enda lagret som ser hubbens RÅA värde.
  // Hubbsidan speglar därför app-sidans regel (B1/A2-2, samma klass som
  // SOG-sentinelen 102,3): cog = null, POSITIONEN BEHÅLLS.
  //
  // VARFÖR null OCH INTE MODULO (361 -> 1): ett värde utanför det kodade
  // intervallet bär ingen riktningsinformation, och att räkna fram en kurs ur
  // det FABRICERAR exakt det som gjorde att normaliseringen 360 -> 0 togs
  // bort (fabricerad nordkurs som matade riktnings- och mållogiken).
  const cog = normalizeCog(num(raw.COG));
  ...
  // J35: samma regel gäller sedan runda 2 även AISStreamClient.
  const navStatus = normalizeNavStatus(num(raw.NAVSTAT));
