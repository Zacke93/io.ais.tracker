...
- **Test:** `tests/boat-near-dedupe-survives-restart.test.js` (PLANERAT NAMN som aldrig landade — låset bärs av andra sviter, se ARCHITECTURE §9; granskning 2026-08-23) — instans A fyrar → ny instans B med samma settings-store → assert ingen dubbel; samt expiry-test (>2 h ska inte blockera).
...
- **Test:** `tests/gps-gate-confirmed-passage-applies.test.js` (PLANERAT NAMN som aldrig landade — låset bärs av andra sviter, se ARCHITECTURE §9; granskning 2026-08-23) (verifierat: failar på nuvarande kod, ska passa efter fix).
...
- **Test:** `tests/stale-guard-vessel-removal.test.js` (PLANERAT NAMN som aldrig landade — låset bärs av andra sviter, se ARCHITECTURE §9; granskning 2026-08-23) (frånkopplad >2 min → "AIS-anslutning saknas"; ansluten → DEFAULT; hash-synk-regress).
