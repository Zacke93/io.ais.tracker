...
(async () => {
  ...
  for (const sample of samples) {
    ...
    try {
      ...
      if (t && t !== lastText) {
        if (idx >= 540 && idx <= 560) {
          const dp = [...svc._directionProgress.keys()];
          const sh = [...svc._stallbackaPhaseHistory.entries()].map(([k, v]) => `${k}:[${[...v]}]`);
          console.log(`[${idx}] ${t.substring(0, 110)}`);
          console.log(`  dirProgress=${JSON.stringify(dp)} stallHistory=${sh.join(',') || 'empty'}`);
        }
        idx++;
        lastText = t;
      }
