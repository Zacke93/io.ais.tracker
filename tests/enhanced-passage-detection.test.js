...
describe('Enhanced Passage Detection', () => {
  ...
  describe('Traditional Close Passage Detection', () => {
    test('should detect close passage with traditional method', () => {
      ...
      const oldVessel = {
        lat: 58.31142, lon: 12.31456, cog: 45, sog: 4.2,
      }; // At bridge center
      const vessel = {
        lat: 58.31150, lon: 12.31470, cog: 45, sog: 4.2,
      }; // Moving away >60m
  ...
  describe('Enhanced Line Crossing Detection', () => {
    test('should detect line crossing with enhanced method', () => {
      ...
      const oldVessel = {
        lat: 58.310800, lon: 12.313500, cog: 200, sog: 3.8,
      };
      const vessel = {
        lat: 58.311800, lon: 12.315500, cog: 30, sog: 3.5,
      };
    ...
    test('should handle relaxed line crossing for maneuvering boats', () => {
      ...
      const oldVessel = {
        lat: 58.310000, lon: 12.313000, cog: 180, sog: 4.0,
      };
      const vessel = {
        lat: 58.312000, lon: 12.316000, cog: 45, sog: 3.8,
      };
  ...
  describe('Progressive Distance Detection', () => {
    test('should detect moderate distance passages', () => {
      const oldVessel = {
        lat: 58.310900, lon: 12.313800, cog: 45, sog: 3.5,
      };
      const vessel = {
        lat: 58.311600, lon: 12.315200, cog: 45, sog: 3.5,
      };
  ...
  describe('Direction Change Passage Detection', () => {
    test('should detect passage when boat changes direction near bridge', () => {
      ...
      const oldVessel = {
        lat: 58.311100, lon: 12.314100, cog: 180, sog: 4.0,
      }; // Close to bridge, heading south
      const vessel = {
        lat: 58.311500, lon: 12.314900, cog: 30, sog: 3.8,
      }; // Past bridge, changed to NE
  ...
  describe('Stallbackabron Special Cases', () => {
    test('should handle Stallbackabron special detection', () => {
      const oldVessel = {
        lat: 58.311350, lon: 12.314400, cog: 45, sog: 3.2,
      };
      const vessel = {
        lat: 58.311500, lon: 12.314700, cog: 45, sog: 3.2,
      };
  ...
  describe('False Positive Prevention', () => {
    test('should not detect passage when boat approaches but does not pass', () => {
      ...
      const oldVessel = {
        lat: 58.308000, lon: 12.311000, cog: 45, sog: 3.0,
      }; // Very far from bridge
      const vessel = {
        lat: 58.308500, lon: 12.311500, cog: 45, sog: 3.0,
      }; // Still far, parallel movement
    ...
    test('should not detect passage for distant movements', () => {
      ...
      const oldVessel = {
        lat: 58.309000, lon: 12.312000, cog: 45, sog: 4.0,
      };
      const vessel = {
        lat: 58.309500, lon: 12.312500, cog: 45, sog: 4.0,
      };
  ...
  describe('Invalid Input Handling', () => {
    ...
    test('should handle invalid coordinates', () => {
      const oldVessel = {
        lat: NaN, lon: 12.314000, cog: 45, sog: 3.0,
      };
      const vessel = {
        lat: 58.311000, lon: 12.314500, cog: 45, sog: 3.0,
      };
  ...
  describe('Target Bridge vs Intermediate Bridge', () => {
    test('should work equally well for target bridges like Klaffbron', () => {
      const oldVessel = {
        lat: 58.284000, lon: 12.283800, cog: 45, sog: 3.8,
      };
      const vessel = {
        lat: 58.284200, lon: 12.284100, cog: 45, sog: 3.8,
      };
