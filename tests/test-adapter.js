...
class TestAdapter extends EventEmitter {
  ...
  _processBoatUpdate(boat) {
    ...
    if (userBridgesInRange.length > 0) {
      ...
      // Set wasInsideTarget if within approach radius
      if (closest.distance < APPROACH_RADIUS) {
    ...
    } else if (boat.targetDistance && boat.targetDistance > 100 && boat.sog < 0.2) {
      // Distant slow boats should be filtered
      if (!boat.protectionZone) {
        boat.status = 'idle';
    ...
    if (boat.targetBridge) {
      // Check for under-bridge
      if (boat.targetDistance < UNDER_BRIDGE_DISTANCE) {
      ...
      // Check for bridge passage
      if (boat.wasInsideTarget && boat.targetDistance > UNDER_BRIDGE_DISTANCE && 
          boat.status !== 'passed' && boat.targetBridge) {
        boat.status = 'passed';
  ...
  _updateActiveBridgesTag() {
    ...
    const targetBoats = this._boats.filter(b => 
      b.targetBridge && (b.status === 'approaching' || b.status === 'waiting' || b.status === 'under-bridge')
    );
      ...
      if (underBridge) {
        ...
        if (primaryBoat) {
          if (primaryBoat.status === 'waiting') {
            ...
            if (primaryBoat.eta === 0) {
              // Very close or slow - show waiting
              bridgeText = `${primaryBoat.name} väntar vid ${primaryBridge}`;
            } else if (context) {
          ...
          if (targetBoats.length > 1) {
            bridgeText = bridgeText.replace('En båt', `${targetBoats.length} båtar`);
          }
        }
      }
    }
